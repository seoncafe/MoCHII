...
!    call random_number(r)   to call as a subroutine
!    r = rand_number()       to call as a function
...
!-----
  ...
  !   module procedure init_random_kiss_seed
...
!----------------------------------
  ...
  subroutine random_kiss_seed(size,put,get)
    ...
    integer, optional, intent(in)  :: put(:)
    integer, optional, intent(out) :: get(:)
    if (.not.(present(size) .or. present(put) .or. present(get))) then
       call init_random_kiss_seed()
    else
...
!----------
  subroutine random_mt_seed_(seed_size,put,get)
    ...
    if (present(put)) then
       ...
       mt(0:n-1) = put(1:n)
       ! mti = 0 means the seeds were initialized.
       mti       = 0
    endif
    if (present(get)) then
