! MoCHII: copied from MoCafe_v2.00/src/memory_mod_mpi.f90 (2026-07-11)
module memory_mod
  ...
  use mpi
  !-- 2023-01-23, optimized reduce_mem routines. (they are slightly faster than before.)
  !-- 2023-01-17. bug-fixed in reduce_mem routines when shared_mem=.true. (MPI_BARRIER should be added.)
...
contains
  ...
  if (.not.use_shared_memory) then
     nsize = size(array)
     if (h_rank == 0) then
        call MPI_REDUCE(MPI_IN_PLACE,array(:),nsize,MPI_REAL8,MPI_SUM,0,hostcomm,ierr)
     else
        call MPI_REDUCE(array(:),           0,nsize,MPI_REAL8,MPI_SUM,0,hostcomm,ierr)
     endif
  endif
  if (h_rank == 0 .and. num_hosts > 1) then
     nsize = size(array)
     if (p_rank == 0) then
        call MPI_REDUCE(MPI_IN_PLACE,array(:),nsize,MPI_REAL8,MPI_SUM,0,same_hrank_comm,ierr)
     else
        call MPI_REDUCE(array(:),           0,nsize,MPI_REAL8,MPI_SUM,0,same_hrank_comm,ierr)
     endif
  endif
  call MPI_BARRIER(MPI_COMM_WORLD, ierr)
  ...
  if (.not.use_shared_memory) then
     ...
     do k=1, size(array(1,1,:))
        if (h_rank == 0) then
           call MPI_REDUCE(MPI_IN_PLACE,array(:,:,k),nsize,MPI_REAL8,MPI_SUM,0,hostcomm,ierr)
        else
           call MPI_REDUCE(array(:,:,k),           0,nsize,MPI_REAL8,MPI_SUM,0,hostcomm,ierr)
        endif
     enddo
  endif
  if (h_rank == 0 .and. num_hosts > 1) then
     ...
     do k=1, size(array(1,1,:))
        if (p_rank == 0) then
           call MPI_REDUCE(MPI_IN_PLACE,array(:,:,k),nsize,MPI_REAL8,MPI_SUM,0,same_hrank_comm,ierr)
        else
           call MPI_REDUCE(array(:,:,k),           0,nsize,MPI_REAL8,MPI_SUM,0,same_hrank_comm,ierr)
        endif
     enddo
  endif
  call MPI_BARRIER(MPI_COMM_WORLD, ierr)
  ...
  if (.not.use_shared_memory) then
     ...
     do j=1, size(array(1,1,:,1))
        if (h_rank == 0) then
           call MPI_REDUCE(MPI_IN_PLACE,array(:,:,j,k),nsize,MPI_REAL8,MPI_SUM,0,hostcomm,ierr)
        else
           call MPI_REDUCE(array(:,:,j,k),           0,nsize,MPI_REAL8,MPI_SUM,0,hostcomm,ierr)
        endif
     enddo
     enddo
  endif
  if (h_rank == 0 .and. num_hosts > 1) then
     ...
     do j=1, size(array(1,1,:,1))
        if (p_rank == 0) then
           call MPI_REDUCE(MPI_IN_PLACE,array(:,:,j,k),nsize,MPI_REAL8,MPI_SUM,0,same_hrank_comm,ierr)
        else
           call MPI_REDUCE(array(:,:,j,k),           0,nsize,MPI_REAL8,MPI_SUM,0,same_hrank_comm,ierr)
        endif
     enddo
     enddo
  endif
  ...
  if (.not.use_shared_memory) then
     ...
     do i=1, size(array(1,1,:,1,1))
        if (h_rank == 0) then
           call MPI_REDUCE(MPI_IN_PLACE,array(:,:,i,j,k),nsize,MPI_REAL8,MPI_SUM,0,hostcomm,ierr)
        else
           call MPI_REDUCE(array(:,:,i,j,k),           0,nsize,MPI_REAL8,MPI_SUM,0,hostcomm,ierr)
        endif
     enddo
     enddo
     enddo
  ...
  if (h_rank == 0 .and. num_hosts > 1) then
     ...
     do i=1, size(array(1,1,:,1,1))
        if (p_rank == 0) then
           call MPI_REDUCE(MPI_IN_PLACE,array(:,:,i,j,k),nsize,MPI_REAL8,MPI_SUM,0,same_hrank_comm,ierr)
        else
           call MPI_REDUCE(array(:,:,i,j,k),           0,nsize,MPI_REAL8,MPI_SUM,0,same_hrank_comm,ierr)
        endif
     enddo
     enddo
     enddo
  ...
  if (.not.use_shared_memory) then
     nsize = size(array)
     if (h_rank == 0) then
        call MPI_REDUCE(MPI_IN_PLACE,array(:),nsize,MPI_REAL4,MPI_SUM,0,hostcomm,ierr)
     else
        call MPI_REDUCE(array(:),           0,nsize,MPI_REAL4,MPI_SUM,0,hostcomm,ierr)
     endif
  endif
  if (h_rank == 0 .and. num_hosts > 1) then
     nsize = size(array)
     if (p_rank == 0) then
        call MPI_REDUCE(MPI_IN_PLACE,array(:),nsize,MPI_REAL4,MPI_SUM,0,same_hrank_comm,ierr)
     else
        call MPI_REDUCE(array(:),           0,nsize,MPI_REAL4,MPI_SUM,0,same_hrank_comm,ierr)
     endif
  endif
  call MPI_BARRIER(MPI_COMM_WORLD, ierr)
  ...
  if (.not.use_shared_memory) then
     ...
     do k=1, size(array(1,1,:))
        if (h_rank == 0) then
           call MPI_REDUCE(MPI_IN_PLACE,array(:,:,k),nsize,MPI_REAL4,MPI_SUM,0,hostcomm,ierr)
        else
           call MPI_REDUCE(array(:,:,k),           0,nsize,MPI_REAL4,MPI_SUM,0,hostcomm,ierr)
        endif
     enddo
  endif
  if (h_rank == 0 .and. num_hosts > 1) then
     ...
     do k=1, size(array(1,1,:))
        if (p_rank == 0) then
           call MPI_REDUCE(MPI_IN_PLACE,array(:,:,k),nsize,MPI_REAL4,MPI_SUM,0,same_hrank_comm,ierr)
        else
           call MPI_REDUCE(array(:,:,k),           0,nsize,MPI_REAL4,MPI_SUM,0,same_hrank_comm,ierr)
        endif
     enddo
  endif
  call MPI_BARRIER(MPI_COMM_WORLD, ierr)
  ...
  if (.not.use_shared_memory) then
     ...
     do j=1, size(array(1,1,:,1))
        if (h_rank == 0) then
           call MPI_REDUCE(MPI_IN_PLACE,array(:,:,j,k),nsize,MPI_REAL4,MPI_SUM,0,hostcomm,ierr)
        else
           call MPI_REDUCE(array(:,:,j,k),           0,nsize,MPI_REAL4,MPI_SUM,0,hostcomm,ierr)
        endif
     enddo
     enddo
  endif
  if (h_rank == 0 .and. num_hosts > 1) then
     ...
     do j=1, size(array(1,1,:,1))
        if (p_rank == 0) then
           call MPI_REDUCE(MPI_IN_PLACE,array(:,:,j,k),nsize,MPI_REAL4,MPI_SUM,0,same_hrank_comm,ierr)
        else
           call MPI_REDUCE(array(:,:,j,k),           0,nsize,MPI_REAL4,MPI_SUM,0,same_hrank_comm,ierr)
        endif
     enddo
     enddo
  endif
  ...
  if (.not.use_shared_memory) then
     ...
     do i=1, size(array(1,1,:,1,1))
        if (h_rank == 0) then
           call MPI_REDUCE(MPI_IN_PLACE,array(:,:,i,j,k),nsize,MPI_REAL4,MPI_SUM,0,hostcomm,ierr)
        else
           call MPI_REDUCE(array(:,:,i,j,k),           0,nsize,MPI_REAL4,MPI_SUM,0,hostcomm,ierr)
        endif
     enddo
     enddo
     enddo
  ...
  if (h_rank == 0 .and. num_hosts > 1) then
     ...
     do i=1, size(array(1,1,:,1,1))
        if (p_rank == 0) then
           call MPI_REDUCE(MPI_IN_PLACE,array(:,:,i,j,k),nsize,MPI_REAL4,MPI_SUM,0,same_hrank_comm,ierr)
        else
           call MPI_REDUCE(array(:,:,i,j,k),           0,nsize,MPI_REAL4,MPI_SUM,0,same_hrank_comm,ierr)
        endif
     enddo
     enddo
     enddo
  ...
  if (.not.use_shared_memory) then
     nsize = size(array)
     if (h_rank == 0) then
        call MPI_REDUCE(MPI_IN_PLACE,array(:),nsize,MPI_INTEGER8,MPI_SUM,0,hostcomm,ierr)
     else
        call MPI_REDUCE(array(:),           0,nsize,MPI_INTEGER8,MPI_SUM,0,hostcomm,ierr)
     endif
  endif
  if (h_rank == 0 .and. num_hosts > 1) then
     nsize = size(array)
     if (p_rank == 0) then
        call MPI_REDUCE(MPI_IN_PLACE,array(:),nsize,MPI_INTEGER8,MPI_SUM,0,same_hrank_comm,ierr)
     else
        call MPI_REDUCE(array(:),           0,nsize,MPI_INTEGER8,MPI_SUM,0,same_hrank_comm,ierr)
     endif
  endif
  call MPI_BARRIER(MPI_COMM_WORLD, ierr)
  ...
  if (.not.use_shared_memory) then
     ...
     do k=1, size(array(1,1,:))
        if (h_rank == 0) then
           call MPI_REDUCE(MPI_IN_PLACE,array(:,:,k),nsize,MPI_INTEGER8,MPI_SUM,0,hostcomm,ierr)
        else
           call MPI_REDUCE(array(:,:,k),           0,nsize,MPI_INTEGER8,MPI_SUM,0,hostcomm,ierr)
        endif
     enddo
  endif
  if (h_rank == 0 .and. num_hosts > 1) then
     ...
     do k=1, size(array(1,1,:))
        if (p_rank == 0) then
           call MPI_REDUCE(MPI_IN_PLACE,array(:,:,k),nsize,MPI_INTEGER8,MPI_SUM,0,same_hrank_comm,ierr)
        else
           call MPI_REDUCE(array(:,:,k),           0,nsize,MPI_INTEGER8,MPI_SUM,0,same_hrank_comm,ierr)
        endif
     enddo
  endif
  call MPI_BARRIER(MPI_COMM_WORLD, ierr)
  ...
  if (.not.use_shared_memory) then
     nsize = size(array)
     if (h_rank == 0) then
        call MPI_REDUCE(MPI_IN_PLACE,array(:),nsize,MPI_INTEGER4,MPI_SUM,0,hostcomm,ierr)
     else
        call MPI_REDUCE(array(:),           0,nsize,MPI_INTEGER4,MPI_SUM,0,hostcomm,ierr)
     endif
  endif
  if (h_rank == 0 .and. num_hosts > 1) then
     nsize = size(array)
     if (p_rank == 0) then
        call MPI_REDUCE(MPI_IN_PLACE,array(:),nsize,MPI_INTEGER4,MPI_SUM,0,same_hrank_comm,ierr)
     else
        call MPI_REDUCE(array(:),           0,nsize,MPI_INTEGER4,MPI_SUM,0,same_hrank_comm,ierr)
     endif
  endif
  call MPI_BARRIER(MPI_COMM_WORLD, ierr)
  ...
  if (.not.use_shared_memory) then
     ...
     do k=1, size(array(1,1,:))
        if (h_rank == 0) then
           call MPI_REDUCE(MPI_IN_PLACE,array(:,:,k),nsize,MPI_INTEGER4,MPI_SUM,0,hostcomm,ierr)
        else
           call MPI_REDUCE(array(:,:,k),           0,nsize,MPI_INTEGER4,MPI_SUM,0,hostcomm,ierr)
        endif
     enddo
  endif
  if (h_rank == 0 .and. num_hosts > 1) then
     ...
     do k=1, size(array(1,1,:))
        if (p_rank == 0) then
           call MPI_REDUCE(MPI_IN_PLACE,array(:,:,k),nsize,MPI_INTEGER4,MPI_SUM,0,same_hrank_comm,ierr)
        else
           call MPI_REDUCE(array(:,:,k),           0,nsize,MPI_INTEGER4,MPI_SUM,0,same_hrank_comm,ierr)
        endif
     enddo
  endif
  call MPI_BARRIER(MPI_COMM_WORLD, ierr)
  ...
  do i=1, num_windows
     if (win == windows(i) .and. windows(i) /= MPI_WIN_NULL) then
        !--- "windows(i)" should be used in the followings to make it equal to MPI_WIN_NULL. (comment added, 2023.01.17).
        call MPI_WIN_FENCE(assert,windows(i),ierr)
        call MPI_WIN_FREE(windows(i),ierr)
        exit
     endif
