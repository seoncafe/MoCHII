...
\item \textbf{Absorption competition.}  With \code{par\%ion\_add\_dust},
  ...
  reradiate it in Section~\ref{sec:sedust}; the astrodust and DL07 models
  are extended below the T-matrix table's $0.0912\,\mu$m end down to
  $1.24/E_{\max}$ using their own dielectric functions (DH21 astrodust,
  D03 silicate and graphite), and each optics module refuses a wavelength
  its dielectric table does not cover rather than let a clamped refractive
  index pass as physics.  The Zubko model is not extended and needs no
  extension: its own DustEM optics tables span
...
\code{kext\_astrodust\_MW\_euv.dat} (1719 wavelengths,
$1.0{\times}10^{-4}$--$3.98{\times}10^{4}\,\mu$m),
\code{kext\_dl07\_MW\_euv.dat} (1761,
$6.2{\times}10^{-5}$--$3.98{\times}10^{4}\,\mu$m), and
...
T-matrix short end $0.0912\,\mu$m:
\begin{center}\small
...
\midrule
astrodust, $E_{\max} = 100$~eV & 4.9e--13 & 4.2e--13 & 1.2e--2 & 3.3e--3
   & 4.1e--6 & 2.8e--14 \\
astrodust, $E_{\max} = 150$~eV & 4.9e--13 & 4.2e--13 & 8.2e--3 & 9.7e--3
   & 2.0e--5 & 2.8e--14 \\
astrodust, no extension        & 4.9e--13 & 4.2e--13 & ---     & ---
   & ---     & 2.8e--14 \\
dl07, $E_{\max} = 100$~eV      & 4.7e--13 & 3.3e--13 & 1.1e--3 & 2.2e--4
   & 3.9e--6 & 7.0e--15 \\
zubko                          & 4.9e--13 & 2.7e--13 & 4.8e--13 & 2.8e--13
   & 2.9e--14 & 1.4e--14 \\
\bottomrule
\end{tabular}
\end{center}
At $\lambda \ge 0.0912\,\mu$m the model grid \emph{is} the table grid
(Zubko's grid is its own DustEM table), so the served value is the table
value and the only difference is the 13 significant digits the tables are
written with.  Below it, on the EUV extension, the extended nodes are not
the table's nodes and the interpolation enters at the percent level at a
single wavelength --- but those departures alternate in sign and cancel in
the two integrals the transport is actually built on: the band-averaged
$s_{\rm abs}$ over 13.598--100~eV agrees to $4{\times}10^{-6}$ and the
normalization $C_{\rm ext}(\lambda_{\rm ref} = 0.55\,\mu$m) to
$3{\times}10^{-14}$, two to five orders below the single wavelengths they
are made of.
...
departs by $O(1)$ at $\lambda \ge 0.0912\,\mu$m, where the two are otherwise
equal to 13 digits.
...
\item \textbf{Shape from the local field.}  For each leaf, the band
  ...
  --- for astrodust the native 1129-point Q-table grid,
  0.0912--$4{\times}10^4\,\mu$m, or, when \code{par\%ion\_add\_dust}
  extends it to $1.24/E_{\max}$, 1302 points from
  $0.0124\,\mu$m at $E_{\max} = 100$~eV.  With the extension the whole
  transported band falls on the grid and the grains absorb it with their own
  cross sections at their own wavelengths; only in an emission-only run
  (\code{ion\_add\_dust} off, native grid) is the part of $J$ below
  $0.0912\,\mu$m deposited into the shortest SEDust bin, which conserves the
  absorbed power but slightly underestimates the hardness of the stochastic
  temperature spikes.  SEDust then
...
\item \textbf{The single-photon bound is a property of the field.}  The
  ...
  Lyman limit --- and measuring the grid there would raise the bound by that
  factor with no photon to justify it, coarsening the enthalpy bins (a fixed
  bin count spread over a 91-fold wider window).  The threshold is
