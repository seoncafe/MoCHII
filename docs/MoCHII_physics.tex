...
the MOCASSIN Lexington benchmarks ($T_e = 8210$~K at HII40 and 7024~K at
HII20 with the density-suppressed cooling default, matching a current
\textsc{Cloudy} c25.00 run to 0.4~K and 1.6\%; the zero-density limit
...
$n$-level model (only C\,\textsc{i} and N\,\textsc{i}) has
$f_{\rm cov}=0$ and keeps the plain fit.  This density-suppressed cooling
...
\midrule
HII20 & 7024 K & 6376 K & 6910 K & 6402--6749 K & --- \\
HII40 & 8210 K & 7894 K & 8210 K & 7720--8199 K & 8176 K \\
\bottomrule
\end{tabular}
\end{center}
With the density-suppressed cooling default (\code{par\%cooling\_model =
local\_ne}) MoCHII gives $T_e = 8210$~K (HII40) and 7024~K (HII20),
matching a current \textsc{Cloudy} c25.00 run (8210 and 6910~K) to 0.4~K
and 1.6\%.  Both modern codes sit a few hundred K \emph{above} the
...
\code{local\_ne} default) raises them to 8210 and 7024~K.  The former
...
\end{tabular}
\end{center}
The classic dusty reduction ($R/R_0 = 0.724$) is reproduced within the
...
\item The thermal-loop line cooling is suppressed to the local $n_e$ by
  ...
  of Section~\ref{sec:cool}; the one term still in its low-density limit
  is the H-impact [C\,\textsc{ii}] 158\,$\mu$m / [O\,\textsc{i}]
  63\,$\mu$m PDR cooling (\code{metal\_cooling\_H}, weighted by
  $x_{\rm HI}$, so $\sim$0 in the ionized benchmark zones but not yet
  saturated in dense PDR gas).  Ar\,\textsc{ii} carries no cooling or
  lines (no CHIANTI v11 level data).
...
\item The MOCASSIN Lexington benchmark temperatures are resolved
  ...
  MoCHII (HII40 8210~K, HII20 7024~K) matches a current \textsc{Cloudy}
  c25.00 run to 0.4~K and 1.6\%, the earlier excess having been traced to
