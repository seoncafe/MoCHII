...
the MOCASSIN Lexington benchmarks ($T_e = 8170$~K at HII40 and 6926~K at
HII20 with the density-suppressed cooling default, matching a current
\textsc{Cloudy} c25.00 run to $0.49\%$ and $0.24\%$; the zero-density limit
...
choice changes the resolution of the saved $J_E$ tally and of the rate
diagnostics only; it cannot move the physical solution.
...
\code{par\%recomb\_model}.  The default \code{'badnell\_milne'} takes the
...
names the two determinations the default combines --- Badnell's total and
the Milne ground level --- and so records that $\alpha_B$ here is a
difference rather than a fit of its own.  Because $\alpha_1$ and the
...
With the shipped abundances 29 of these reactions are active --- 7 in the
$\mathrm{H}^+ \to \mathrm{H}^0$ direction (C, N, O, Mg, Si, S, Fe, all at
the neutral stage) and 22 in the reverse --- and every one of them enters
hydrogen's balance as well as the metal's
(\S\ref{sec:ioneq}, \code{par\%charge\_exchange}).
...
$R_{\rm cx}^{\rm out}$ ionizes an H$^0$.  Eliminating
$n_{\rm HII} = n_{\rm H}(1-\xHI)$ keeps the closed form,
\begin{equation}
...
$\alpha_{\rm H} n_e$ does.  The two sums come from the same walk of the
...
$+2.2$~K at HII40 and $+2.1$~K at HII20 (to the headline 8170 and 6926~K),
...
MoCHII therefore does not substitute.  \code{hydrogen\_neutral\_fraction}
solves the balance at the current $n_e$ for its root, with the whole metal
cascade re-evaluated at each trial $x_{\rm HI}$.  The root is bracketed in
$[0,1]$ \emph{by construction} --- $R(0) = -(\alpha_H n_e + R_{\rm in}) < 0$
and $R(1) = \Gamma_H + (C_H+\alpha_H)n_e + R_{\rm out} > 0$ --- so Illinois
iteration keeps the bracket at every step and the fraction cannot leave the
physical range, which is the one property the plain substitution had and a
bare secant would forfeit.  One call now reaches the converged state to
$3.6\times10^{-11}$ at every $x_{\rm HII}$ down to $0.02$, against
$1.8\times10^{-3}$ for substitution, and on HII20 the printed balance
residual falls from a maximum of $4.6\times10^{-3}$ to $1.3\times10^{-6}$
with the volume-weighted mean from $4.7\times10^{-4}$ to
$4.5\times10^{-9}$.  The benchmark numbers above move by $0.01$~K, so the
...
\code{docs/ION\_BALANCE\_CONVERGENCE\_PLAN.md}.
...
a printed diagnostic and does not gate convergence.
...
(\S\ref{sec:ioneq}).  Adding an ion is one entry in the data generators
...
Concretely, \code{species\_opacity\_add} sums over \emph{all}
...
$+2.1$~K at HII20, to the headline 8170 and 6926~K.
...
\item \textbf{Absorption competition.}  With \code{par\%ion\_add\_dust},
  ...
  (\lambda_{\rm ref})$.  By default these come from the grain model named
  by \code{par\%dust\_model},
  so that the power a cell absorbs is set by the same grains that
  reradiate it in Section~\ref{sec:sedust}.  No model is extended: the
  astrodust and DL07 grids come from the random-orientation T-matrix table,
  which spans $1.0{\times}10^{-4}$--$3.981{\times}10^{4}\,\mu$m in 1762
  wavelengths, i.e.\ 12.4~keV down to the far infrared, so below the Lyman
  limit the optics are the same $b/a = 1.4$ oblate spheroid on the DH21
  dielectric function as above it (with the Rayleigh, $x < 0.1$, and
  ...
  volume-equivalent-sphere approximation.  \code{grain\_model\_mod} still asks
  for the grid down to $1.24/E_{\max}$, but SEDust prepends points only when
  that is \emph{shorter} than the table's first wavelength: at
  $E_{\max} = 100$~eV it is not ($0.0124\,\mu$m against
  $1.0{\times}10^{-4}\,\mu$m), and an extension can arise only above
  $E_{\max} = 12398.4$~eV.  There astrodust refuses to build one --- that
  band's optics are the same spheroid, and this tree carries the Q table
  without the T-matrix solver --- and the refusal is turned into a message
  naming \code{par\%eion\_max}; a clamped refractive index or a silent
  equal-volume sphere is never passed off as physics.  The Zubko model needs no
  extension either: its own DustEM optics tables span
  0.001--$10^4\,\mu$m (1.24~keV down to $1.24{\times}10^{-4}$~eV), so the
  whole ionizing band is already tabulated, which is why
  \code{build\_zubko} takes no \code{lam\_min}.  Whichever model is named,
...
for it.  SEDust's \code{dust\_extinction} returns the \emph{precomputed}
size-integrated curve of a \code{SEDust/data/kext\_*.dat} table, which the
builder loaded, interpolated onto the model's own wavelength grid
(log~$\lambda$--log~$C$ for the cross sections, linear in log~$\lambda$ for
$\langle\cos\rangle$, and the table value itself wherever a model wavelength
coincides with a table wavelength).  The size integral
\begin{equation}
...
\end{equation}
is the calculation that \emph{wrote} those tables; it remains reachable as
SEDust's \code{size\_integrated\_extinction}.  Which table a run serves is
\code{par\%sed\_kext}: blank (the default) takes SEDust's own standard table
for the named \code{par\%dust\_model}, so those file names are written down in
one place; naming a file instead makes it mandatory, and the build stops with
that file named if it cannot be read.  The standard tables are
\code{kext\_astrodust\_MW\_euv.dat} (1762 wavelengths,
$1.0{\times}10^{-4}$--$3.981{\times}10^{4}\,\mu$m),
\code{kext\_dl07\_MW\_euv.dat} (1823,
$6.2054{\times}10^{-5}$--$3.981{\times}10^{4}\,\mu$m), and
\code{kext\_zubko\_BARE\_GR\_S.dat} (1201, $10^{-3}$--$10^{4}\,\mu$m).

Serving a table instead of redoing the integral is exact where the model
grid and the table grid coincide and interpolated where they do not, and
the gate \code{tests/grain\_kext} measures both against
\code{size\_integrated\_extinction} over the model as MoCHII builds it.
Cross sections are compared relative to themselves (they set rates); the
albedo $a$ and the product $a\langle\cos\rangle$ absolutely, because the
transport uses them as the probability that an interaction scatters and the
forward displacement it carries.  The worst departure on each side of the
Lyman limit $0.0912\,\mu$m, which the gate keeps as its reporting boundary:
\begin{center}\small
\begin{tabular}{@{}lcccccc@{}}
\toprule
 & \multicolumn{2}{c}{$\lambda \ge 0.0912\,\mu$m}
 & \multicolumn{2}{c}{$\lambda < 0.0912\,\mu$m} & band & \\
model as built & $C_{\rm ext}, C_{\rm abs}$ & $a$, $a\langle\cos\rangle$
 & $C_{\rm ext}, C_{\rm abs}$ & $a$, $a\langle\cos\rangle$
 & $s_{\rm abs}$ & $C_{\rm ext}(\lambda_{\rm ref})$ \\
\midrule
astrodust, $E_{\max} = 100$~eV     & 4.91e--13 & 4.24e--13 & 4.84e--13 & 8.13e--13
   & 1.95e--14 & 2.83e--14 \\
astrodust, $E_{\max} = 150$~eV     & 4.91e--13 & 4.24e--13 & 4.84e--13 & 8.13e--13
   & 1.95e--14 & 2.83e--14 \\
astrodust, no \code{lam\_min}      & 4.91e--13 & 4.24e--13 & 4.84e--13 & 8.13e--13
   & 1.95e--14 & 2.83e--14 \\
dl07, $E_{\max} = 100$~eV          & 4.65e--13 & 3.07e--13 & 3.21e--12 & 1.50e--12
   & 2.95e--14 & 2.12e--14 \\
zubko                              & 4.91e--13 & 2.72e--13 & 4.83e--13 & 2.77e--13
   & 2.89e--14 & 1.42e--14 \\
\midrule
tolerance                          & 1e--10 & 1e--10 & 1e--9 & 1e--9
   & 1e--4 & 1e--9 \\
\bottomrule
\end{tabular}
\end{center}
Nothing here is interpolation any more: the model grid \emph{is} the table
grid on both sides of the Lyman limit, so the served value is the table value
and the only difference is the 13 significant digits the tables are written
with.  The three astrodust rows are identical because they are the same grid
--- asking for $1.24/E_{\max}$ at 100~eV, at 150~eV, or not asking at all all
return the native 1762-point table.  DL07 is the one case where the two grids
are not the same object: its kext table is the finer of the two (1823 points
from $6.2054{\times}10^{-5}\,\mu$m against the model's 1762 from
$1.0{\times}10^{-4}\,\mu$m), but the model's nodes fall on table nodes to
rounding, which is why its numbers sit at $10^{-12}$ rather than $10^{-13}$.
The two integrals the transport is actually built on follow: the band-averaged
$s_{\rm abs}$ over 13.598--100~eV agrees to $2$--$3{\times}10^{-14}$ and the
normalization $C_{\rm ext}(\lambda_{\rm ref} = 0.55\,\mu$m) to
$1$--$3{\times}10^{-14}$.  Because no interpolation is left to bound, the
tolerance below the Lyman limit was tightened from $3{\times}10^{-2}$ to
$10^{-9}$.

What the table presumes is that it was computed from that model's
\emph{default} inputs.  Changing \code{par\%sed\_sizedist},
\code{par\%sed\_qtable}, \code{par\%sed\_dl07\_sdindex} or
\code{par\%sed\_zubko\_config} moves the grains that emit without moving the
curve that extinguishes, and the two then no longer describe the same dust.
That is exactly what the gate catches: a table belonging to other grains
departs by $O(1)$ where the two are otherwise equal to 13 digits.
...
size-integrated curve is unchanged to rounding (worst $1.4{\times}10^{-10}$ on
$C_{\rm ext}$); below it, it was recomputed.  Over the band MoCHII transports,
13.598--100~eV (137 grid points), the new curve departs from the old by at most
...
band integrals move $+0.459\%$ ($C_{\rm ext}$) and $+0.910\%$
($C_{\rm abs}$).  Far larger differences, up to a factor $1300$, sit at
$1.0$--$1.1{\times}10^{-4}\,\mu$m --- the 12.4~keV end of the table, outside
anything MoCHII transports --- and the two must not be quoted together.
Building the longer grid costs 156.4~s (astrodust), about 73~s (DL07) and
0.55~s (Zubko) at \code{par\%sed\_NT} $= 200$, paid once per rank at startup.
...
\item \textbf{Shape from the local field.}  For each leaf, the band
  ...
  --- for astrodust and DL07 the native 1762-point Q-table grid,
  $1.0{\times}10^{-4}$--$3.981{\times}10^{4}\,\mu$m, and for Zubko its own
...
\item \textbf{The single-photon bound is a property of the field.}  The
  ...
  Lyman limit, and the astrodust and DL07 Q-table grid now starts a further
  decade up, at $1.0{\times}10^{-4}\,\mu$m (12.4~keV) --- and measuring the
...
\item \textbf{Launch.}  A leaf is chosen in proportion to its emitted
  ...
  no bin centre enters.  Packets are allocated by stratified sampling of
...
\item \textbf{Transport.}  The packets ride the same routine as the
  ...
  have been counted a second time into ${\rm Heat\_dust}$ and the whole
  infrared field would have entered the Habing $G_0$ integral.
...
\midrule
HII20 & 6926 K & 6376 K & 6910 K & 6402--6749 K & --- \\
HII40 & 8170 K & 7894 K & 8210 K & 7720--8199 K & 8176 K \\
\bottomrule
\end{tabular}
\end{center}
With the density-suppressed cooling default (\code{par\%cooling\_model =
local\_ne}) MoCHII gives $T_e = 8170$~K (HII40) and 6926~K (HII20),
matching a current \textsc{Cloudy} c25.00 run (8210 and 6910~K) to
$0.49\%$ and $0.24\%$.  Both modern codes sit a few hundred K \emph{above} the
...
oxygen charge exchange of Section~\ref{sec:cx} then adds a final $+2$~K,
giving the headline 8170 and 6926~K.  The former
...
\item With the default astrodust or DL07 SEDust optics, the EUV part of the
  leaf field lies on the 1762-point EUV Q-table grid and is not folded into a
  shorter-wavelength bin.  A custom Q/kext table that does not cover the
...
\item The MOCASSIN Lexington benchmark temperatures are resolved
  ...
  MoCHII (HII40 8170~K, HII20 6926~K) matches a current \textsc{Cloudy}
  c25.00 run to $0.49\%$ and $0.24\%$, the earlier excess having been traced
