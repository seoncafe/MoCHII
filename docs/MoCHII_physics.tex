...
The same cancellation has a numerical consequence that is recorded here
rather than hidden: the slope of the substitution map at its fixed point
is $R_{\rm in}/(R_{\rm in} + \alpha_H n_e)$, which tends to unity wherever
$R_{\rm in} \gg \alpha_H n_e$, so in partially neutral gas the cell
iteration is marginally stable.  At $T=10^4$~K, $n_{\rm H}=100$ and the
HII20 abundances it needs 124 passes at $x_{\rm HII}=0.18$, 239 at
$0.10$ and 1285 at $0.021$, against the 200-pass cap; a capped cell leaves
the loop with $x_{\rm HII}$ high by about $8\%$.  Cells with
$x_{\rm HII}\gtrsim 0.1$ converge inside the cap, and the benchmark
diagnostics are weighted by $n_{\rm p}n_{\rm e}\propto x_{\rm HII}^2$, so
the numbers above are unaffected; what is affected is the shadow beyond
the front, which must be resolved before a photodissociation-region result
is quoted from this term.
