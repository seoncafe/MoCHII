...
account for $1.7\%$ of the gap and with the wrong sign.  Four candidates
remain, in decreasing order of plausibility:
\begin{enumerate}
\item the \emph{transport} of the diffuse photons rather than their
  emission --- the one axis never varied in this sequence;
\item the He\,\textsc{i} ground-continuum atomic data.  \code{milne\_setup}
  reports the $\alpha_1$ of the Milne integral disagreeing with the fitted
  $\alpha_A-\alpha_B$ by $2$--$3.4\%$ for He\,\textsc{i}, against $0.5\%$ for
  H\,\textsc{i} and $0.4\%$ for He\,\textsc{ii} --- in exactly the continuum
  whose front is discrepant;
\item collisional redistribution among the $n=2$ He\,\textsc{i} terms, which
  is density dependent and not implemented;
\item a difference outside the diffuse field entirely --- the metal opacity
