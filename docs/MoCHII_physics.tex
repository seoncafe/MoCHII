...
\end{center}
The caveat carried in every header: these are low-density-limit fits and
carry no suppression above $n_{\rm crit}$, so the low-$n_{\rm crit}$
lines ([C\,\textsc{ii}] 158\,$\mu$m, [O\,\textsc{iii}] 52/88\,$\mu$m,
the [O\,\textsc{ii}] and [S\,\textsc{ii}] optical doublets) are
overestimated in dense gas; this is now the leading approximation of the
thermal loop (Section~\ref{sec:hii}).  Density-dependent emissivities
come from the $n$-level data.
