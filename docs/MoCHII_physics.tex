...
the MOCASSIN Lexington benchmarks ($T_e = 8211$~K at HII40 and 7025~K at
HII20 with the density-suppressed cooling default, matching a current
\textsc{Cloudy} c25.00 run to 0.7~K and 1.7\%; the zero-density limit
...
reports it at start-up against the $\alpha_A-\alpha_B$ that sets the packet
count; the two are independent data and are never rescaled into agreement.
They agree to $0.5$--$1.3\%$ for H\,\textsc{i} and $0.4\%$ for
He\,\textsc{ii}, but differ by up to $3.4\%$ for He\,\textsc{i}.

\paragraph{Which side of the He\,\textsc{i} cross-check is the outlier.}
The disagreement is not an implementation error on the cross-section side.
MoCHII's \code{sigma\_HeI} reproduces Verner's own \code{phfit2.f} to a ratio
of $1.00000$ at every energy over $24.6$--$105$~eV, and its threshold value,
$7.436$~Mb, sits $0.5\%$ above the measured $7.40\pm0.07$~Mb of Samson et
al.\ (1994) --- inside the experimental uncertainty, so the normalization is
right and any disagreement would have to lie in the energy dependence.  Nor
is the comparison between unlike quantities: \code{recomb\_mod} forms
$\alpha_B = \alpha_A - \alpha_1$ from Eq.~(\ref{eq:mao}), so the ``rates''
side of the cross-check \emph{is} $\alpha_1$(He\,\textsc{i}) itself, and the
$2$--$3.4\%$ separates two determinations of one number.  A third,
independent determination decides between them: Benjamin, Skillman \& Smits
(1999) with J.~S.\ Mathis, as carried by K.~Wood's \code{ionize}, give
$\alpha_1({\rm He\,\textsc{i}}) = 1.54{\times}10^{-13}(T_4)^{-0.486}$
$\cmc$.
\begin{center}
\begin{tabular}{lrrr}
\toprule
$T$ & BSS99 $+$ JSM & Mao \& Kaastra & Milne integral, Verner $\sigma$ \\
\midrule
...
\end{tabular}
\end{center}
In $10^{-13}\,\cmc$, percentages against the BSS99 column.  The two
recombination determinations cluster within $2.6\%$ of each other while the
Milne integral sits $4.5$--$4.8\%$ above both, so \emph{the Milne integral is
the outlier} --- and He\,\textsc{i} is the only one of the three ground
continua that is not hydrogenic, the two that are agreeing to
$0.5$--$1.3\%$ and $0.4\%$.  The code reports the mismatch at start-up and
rescales neither side, which is the correct handling: it is an inconsistency
between two independent atomic data sets, inherited by every code that pairs
the Verner cross sections with recombination fits, not a defect in either
implementation.
...
is $r({\rm He^0}=0.5) = 4.11970$~pc against Cloudy c25.00's $4.30531$~pc,
i.e.\ $-0.18561$~pc.  The front's sensitivity to the diffuse field is
...
the He\,\textsc{i} atomic data by $3\%$, the size of the entire spread among
its published determinations: reducing $\sigma_{\rm pi}$(He\,\textsc{i})
...
\begin{tabular}{@{}lll@{}}
\toprule
axis & bound measured & to reach $0.18$~pc \\
\midrule
...
Closing $0.18$~pc is $4.4\%$ in radius and $13.7\%$ in volume, which asks for
$Q({\rm He^0})$ higher by ${\sim}14\%$ or $\alpha_B$(He\,\textsc{i}) lower by
${\sim}20\%$.  $Q({\rm He^0})$ is verified to $0.003\%$, and MoCHII's
$\alpha_B$(He\,\textsc{i}) differs from the BSS99 value by $2.9\%$, with no
...
not in dust, not in the source photon budget, and not in the reference
calculation's resolution.  It demands a helium-specific effect at the
$15$--$20\%$ level that is in none of the rates, cross sections or fields
checked.  Two candidates survive:
\begin{enumerate}
\item the helium ionization network itself --- what Cloudy's resolved
  He\,\textsc{i} model atom contains that MoCHII's effective treatment does
  not.  Collisional redistribution among the $n=2$ terms, resolved
  excited-level populations and their feedback on the ionization balance all
  live here.  Deciding it requires reading Cloudy's He\,\textsc{i}
  implementation and comparing network to network, which is a separate task
  and is the open item;
\item a difference outside the helium network entirely --- the metal opacity
...
local\_ne}) MoCHII gives $T_e = 8211$~K (HII40) and 7025~K (HII20),
matching a current \textsc{Cloudy} c25.00 run (8210 and 6910~K) to 0.7~K
and 1.7\%.  Both modern codes sit a few hundred K \emph{above} the
...
\code{local\_ne} default) raises them to 8211 and 7025~K.  The former
...
\item The MOCASSIN Lexington benchmark temperatures are resolved
  ...
  MoCHII (HII40 8211~K, HII20 7025~K) matches a current \textsc{Cloudy}
  c25.00 run to 0.7~K and 1.7\%, the earlier excess having been traced to
  two Tier-1 cooling defects (D1/D2) and the missing density suppression,
