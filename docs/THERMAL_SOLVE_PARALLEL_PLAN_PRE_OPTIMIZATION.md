...
4. Keep the closing `MPI_BARRIER(mpar%hostcomm)` so the shared writes are
   visible on return.
...
This is a performance change, not one of the review findings, and it touches
the routine that the outstanding physics work also touches. Land it only after
the physics fixes already in the working tree are validated and committed;
otherwise a discrepancy cannot be attributed to either change.
