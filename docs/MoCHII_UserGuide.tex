...
\code{ion\_dust\_kext} & '' & optional kext table (e.g.\ \code{data/kext\_albedo\_WD\_MW\_3.1\_60\_D03.all\_2003}).  Blank (the default) takes $C_{\rm ext}$, albedo and $\langle\cos\rangle$ from the grain model named by \code{dust\_model\_sed}, so the transport and the emission refer to the same grains; a file OVERRIDES the model and is then consistent with the emission only if it came from the same grains.  The model must cover the band --- astrodust and DL07 are extended into the EUV from their dielectric functions, Zubko is not \\
