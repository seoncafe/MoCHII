...
10830~\AA\ line-center optical depth:
\begin{verbatim}
...
precision.  \code{tau10830\_map} deposits the line-center opacity
...
$\tau_0 > 1$: the diagnostic assumes optically thin 10830 (line transfer
is out of scope).  The command line takes \code{--column n\_2s3} or
\code{--tau10830 --component doublet --vturb 5}.
...
\code{hei\_meta} & He\,\textsc{i} 2$^3$S metastable: OH18 analytic limits, module $n_3$ vs.\ the Python closed form, $\Phi_3$ from the band tally, and the 10830~\AA\ column/optical-depth diagnostic (conservation + hand integral) \\
\bottomrule
\end{tabular}
\end{center}
