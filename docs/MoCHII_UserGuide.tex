...
\code{use\_master\_slave} & F & how the photons of one transport pass are split over the MPI ranks.  F = static cyclic: rank $r$ takes indices $r{+}1,\,r{+}1{+}n_{\rm proc},\,\ldots$; T = rank 0 becomes the master, serving batches of \code{num\_send\_at\_once} consecutive indices to whichever rank asks next; \textbf{the master transports no photons}, so $n_{\rm proc}-1$ ranks do the work.  Applies to the stellar loop, the diffuse LyC loop and (through the same routine) the peel-off imaging pass; the dust-emission infrared transport and the SEDust solve distribute leaves rather than photons and are unaffected.  Worth $\sim$20\% of the ionizing-band pass at 70 ranks, and it cuts the run-to-run spread of that pass from 28\% to 1.5\%.  Under \code{launch\_sequence='random'} each rank draws from its own stream, so T also selects the realization --- statistically equivalent, not numerically comparable; with \code{'sobol'} the two settings transport the same packet set and differ only by summation order \\
\code{num\_send\_at\_once} & 10000 & photon indices in one served batch (\code{use\_master\_slave=T} only) \\
\end{longtable}
}
...
\code{car\_walk}  & 'dda' & car-grid traversal: \code{'dda'} (default) = dedicated incremental Amanatides--Woo walks ($t_{\rm max}$/$t_\Delta$ per axis, comparisons and additions only --- the fastest car walk); \code{'amr'} = the octree walks with integer index arithmetic replacing the neighbor table, a verification mode that is bit-identical to the single-level octree (\code{grid\_type\,=\,'amr'}) \\
...
\bottomrule
\endlastfoot
\code{dust\_density\_law} & 'global\_dgr' & leaf grey opacity: \code{'none'}, \code{'global\_dgr'} ($n_{\rm H}c_{\rm ext}{\cdot}$DGR), \code{'from\_file'} (ndust column), \code{'laursen09'} (static xHI column), \code{'laursen09\_live'} (computed $x_{\rm HI}$, refreshed each iteration) \\
...
\code{nnu\_ion} & 16 & diagnostic ionizing-energy bins (gates use 32).  Packet energies are sampled from the source CDF and every cross section is evaluated at the sampled energy, so changing this count changes only the resolution of the saved tallies, never the physical solution \\
\code{eion\_min}, \code{eion\_max} & 13.598, 100 & ionizing band edges [eV] \\
\code{ion\_align\_edges} & T & align diagnostic bin edges to the ionization thresholds, so no bin straddles an edge; segments are log-uniform and low-priority metal edges are dropped safely when necessary.  Affects the tally resolution only, not the transport.  Set F for a single log grid \\
...
\code{spectrum\_type} & 'shape\_ev' & column units of every spectrum-file slot (\code{ion\_spectrum}, \code{src\_spectrum\_file}, \code{ext\_spectrum}): \code{'shape\_ev'} (arbitrary, normalized to the scale --- legacy) or a physical type \code{'per\_ev'}/\code{'per\_hz'}/\code{'per\_ang'}/\code{'per\_um'} (absolute); see \S\ref{sec:specfmt} \\
\code{luminosity} & unset & ionizing BAND luminosity [erg/s] of the (single) source; for a \code{'shape\_ev'}/Planck source unset maps to 1.0 (legacy), for a physical-type file unset means DERIVE it from the file integral; with \code{nsource}$\ge 2$ it becomes the sum of \code{src\_lum} (or the equal-split total when those are unset) \\
...
\label{sec:specfmt}
A single parameter, \code{spectrum\_type} (default \code{'shape\_ev'}), fixes
...
\midrule
\code{'shape\_ev'}   & $E$ [eV] $\uparrow$ & arbitrary per eV (shape of $L_E$) & arbitrary per eV \\
...
\code{'shape\_ev'} file must be ascending in $E$, while the physical types
...
\code{'shape\_ev'} keeps the legacy behavior: the shape is normalized to the
...
\medskip\noindent\textbf{\code{'shape\_ev'} is per eV, not variable-agnostic.}
\code{'shape\_ev'} reads its value columns as the shape of the per-eV density
$L_E$ on the $E$~[eV] grid of column~1.  A table of $F_\lambda$ (or
$F_\nu$) values re-gridded onto $E$ under \code{'shape\_ev'} therefore omits the
...
\code{case\_ab} & 'B' & case A/B recombination in the ionization balance, i.e.\ how the ionizing \emph{continuum} is treated ('A' is forced by \code{diffuse\_field}, which transports the ground-state recombination photons) \\
\code{line\_case} & 'B' & case of the SH95 H\,\textsc{i}/He\,\textsc{ii} recombination-\emph{line} tables.  This is set by the optical depth of the Lyman \emph{lines}, a different question from \code{case\_ab}: a nebula whose diffuse continuum is transported still traps its Lyman lines and radiates case-B Balmer lines, so 'B' is the physical default for H\,\textsc{ii} regions.  'A' selects the case-A tables (Lyman-line-thin gas); 'follow' ties the line case to \code{case\_ab} \\
...
\code{cooling\_model} & 'local\_ne' & line cooling in the thermal loop: 'local\_ne' (default) suppresses each ion's cooling fit to the local $n_e$ with a $(T, n_e)$ table built from its $n$-level solve, so the thermal balance and the emergent lines are one physics; 'low\_density' uses the $n_e\to0$ fits directly (the coronal limit, exact in diffuse gas; reproduces earlier gates) \\
...
\code{charge\_exchange} & 'two\_way' & charge exchange $\mathrm{X}^{i+}+\mathrm{H}^+ \leftrightarrow \mathrm{X}^{(i+1)+}+\mathrm{H}^0$ in hydrogen's balance as well as the cascade's, so the charge each reaction moves is conserved in the reacting pair: $n_{\rm HI}(\Gamma_{\rm H}+n_eC_{\rm H}+R_{\rm cx}^{\rm out}) = (\alpha_{\rm H}n_e+R_{\rm cx}^{\rm in})n_{\rm HII}$ with the sums over all 29 active reactions.  'metal\_only' keeps them in the cascade alone (the earlier bookkeeping, which does not conserve charge); it is a reproduction value, not a physics option \\
\end{longtable}
}
{\small
\begin{longtable}{llp{7.2cm}}
...
\code{hei\_metastable} & F & write \code{\_heimeta} (HDF5/FITS): the He\,\textsc{i} 2$^3$S metastable density $n_3$, its photoionization rate $\Phi_3$, plus $n_{\rm H}$, $n_e$, $T_e$, and leaf geometry (the 10833~\AA\ absorption diagnostic; Section~\ref{sec:outputs}).  With \code{add\_fuv} and \code{efuv\_min}=4.78 the soft-UV part of $\Phi_3$ is included \\
...
\code{diffuse\_energy\_model} & 'milne' & spectrum of the three ground continua (needs \code{diffuse\_field}): 'milne' (default) is the detailed-balance free--bound spectrum $E^2\sigma_{\rm pi}(E)e^{-(E-E_{\rm th})/kT}$ with the transport cross sections; 'exponential' the historical $E_{\rm th}+kT[-\ln u]$ approximation (reproduces earlier runs); 'threshold' a sensitivity bracket, not a model \\
\code{hei\_diffuse} & F & fourth diffuse channel (needs \code{diffuse\_field}): He\,\textsc{i} excited-level recombination photons.  The $2^3$S\,:\,$2^1$S\,:\,$2^1$P branch fractions come from one consistent set of $\alpha_{\rm eff}$ (Benjamin, Skillman \& Smits 1999 Table~1 with J.~S.\ Mathis, as in K.~Wood's \code{ionize}) normalized by its own sum --- $0.762 : 0.076 : 0.162$ at 8000~K against the statistical $0.750 : 0.083 : 0.167$ --- and the $2^1$S photons from the Drake et al.\ (1969) two-photon shape.  Correct energy bookkeeping: $+0.12\%$ in energy and $+0.30\%$ in photons per case-B He\,\textsc{ii} recombination, $L({\rm H}\beta)$ within 0.02\%, He split unchanged.  The trapped $2^1$P (584~\AA) photon is split locally between photoionizing hydrogen and collisional conversion $2^1{\rm P}\rightarrow2^1{\rm S}$, with $p_{584}=[1+77x({\rm He^0})/(\sqrt{T}x({\rm H^0}))]^{-1}$ (Wood, Mathis \& Ercolano 2004) --- the same split the He\,\textsc{i} two-photon nebular continuum uses.  At HII40 it lowers $L({\rm H}\beta)$ by 0.2\% and moves the He\,\textsc{i} front inward by 0.0036~pc \\
...
\code{metal\_ne} & T & electrons from the metal cascade join the $n_e$ closure.  Solved as a fixed point, since the stage fractions depend on $n_e$ themselves: a single evaluation at the H/He-only electron density would be the $n_e\to0$ limit, in which nothing recombines and every low-IP metal comes out fully ionized.  Switching it off costs only $0.002\%$ of $\langle T[N_{\rm p}N_{\rm e}]\rangle$ on the Lexington benchmarks --- but beyond the ionization front the metal stages are the ONLY electron source, so there it is not optional \\
\code{metal\_heat} & T & metal photoheating in the thermal balance, from the same $\Gamma$'s the cascade uses.  Switching it off lowers $\langle T[N_{\rm p}N_{\rm e}]\rangle$ by 21.1~K ($0.26\%$) at HII40 and 5.7~K ($0.082\%$) at HII20 \\
...
\code{ion\_dust\_kext} & '' & optional kext table (e.g.\ \code{data/kext\_albedo\_WD\_MW\_3.1\_60\_D03.all\_2003}).  Blank (the default) takes $C_{\rm ext}$, albedo and $\langle\cos\rangle$ from the grain model named by \code{dust\_model}, so the transport and the emission refer to the same grains; a file OVERRIDES the model and is then consistent with the emission only if it came from the same grains.  Taken from the model, the curve is that model's precomputed size-integrated extinction table in \code{SEDust/data/} (\code{kext\_astrodust\_MW\_euv.dat}, \code{kext\_dl07\_MW\_euv.dat}, \code{kext\_zubko\_BARE\_GR\_S.dat}), interpolated onto the model's own wavelength grid (\code{sed\_kext} selects it).  The model must cover the band, and all three do on their own: astrodust and DL07 take their grid from the T-matrix optics table, 1762 wavelengths over $1.0{\times}10^{-4}$--$3.981{\times}10^{4}\,\mu$m, so below the Lyman limit the optics are the $b/a = 1.4$ oblate spheroid itself rather than a sphere approximation, and Zubko's own DustEM optics span 0.001--$10^4\,\mu$m.  Nothing is extended at any energy the code transports; an extension can only be asked for above \code{eion\_max} $= 12398.4$~eV, and astrodust refuses it there with a message naming \code{eion\_max} rather than substitute an equal-volume sphere \\
...
\code{dust\_model} & 'astrodust' & 'astrodust'/'dl07'/'zubko' \\
...
\code{sed\_qtable}, \code{sed\_sizedist} & \code{../tmatrix/output/q\_astrodust\_P0.20\_Fe0.00\_1.400\_euv.dat}, in-tree size distribution & optics/size-distribution tables (relative to \code{sed\_workdir}).  MoCHII is a photoionization code, so the EUV companion Q table is the default: 169 radii, 1762 wavelengths, $1.0\times10^{-4}$--$3.981\times10^4\,\mu$m.  Leave them at the defaults unless you also supply matching kext data: the extinction the transport reads is the model's precomputed size-integrated table, which was computed from these defaults, so changing them moves the grains that emit without moving the curve that extinguishes.  The gate \code{tests/grain\_kext} checks that pairing \\
\code{sed\_dl07\_sdindex}, \code{sed\_dl07\_uisrf} & 7, 1.0 & DL07 only: WD01 size-distribution index (7 = MW $R_V = 3.1$, $b_C = 6{\times}10^{-5}$) and the ISRF intensity setting the PAH ionization fraction; same caution as above \\
\code{sed\_zubko\_config}, \code{sed\_zubko\_dir} & in-tree defaults & Zubko only: ZDA configuration file and the directory of its DustEM optics tables (relative to \code{sed\_workdir}); same caution as above \\
\code{sed\_kext} & '' & the precomputed size-integrated extinction curve SEDust serves to the transport ($\lambda$, albedo, $\langle\cos\rangle$, $C_{\rm ext}/$H, $C_{\rm abs}/$H, $C_{\rm sca}/$H), relative to \code{sed\_workdir}.  Blank (the default) takes SEDust's own standard table for the named \code{dust\_model}, so those file names are written down in one place (the \code{\_euv} in two of them is historical: the tables now span the model's whole grid).  Naming a file makes it MANDATORY (the build stops if it cannot be read) and it must have been computed from the same model and size distribution the emission uses, which is what SEDust's \code{calc\_kext.x} writes.  Distinct from \code{ion\_dust\_kext}, which does not change what SEDust serves but REPLACES the served curve downstream, leaving the emission on other grains \\
...
\code{dust\_emis\_transport} & T & transport the reradiated infrared through the same grain opacity, launching from each leaf in proportion to its emitted power with the wavelength drawn continuously from that leaf's spectrum; what the packets deposit returns to \code{Heat\_dust}.  Set F to post-process the emission only \\
\code{dust\_niter}, \code{dust\_tol} & 2, 1e-3 & maximum transport passes and the relative change in emitted luminosity that ends them; $=1$ transports once with no self-consistency check \\
\code{dust\_no\_photons} & 1e6 & infrared packets per pass \\
\end{longtable}
}
...
\code{peel\_bins} & F & bin-resolved cubes \code{direct\_cube}/\code{scatt\_cube} $(n_x, n_y, n_\nu)$ in addition to the band-integrated channel images (hardness maps, synthetic EUV/FUV photometry); the third (bin) axis is labeled by the \code{E\_bin}/\code{dE\_bin} [eV] and \code{wl\_bin} [\AA] arrays written once into the same file \\
...
wavelength [\AA], $L$ [erg\,s$^{-1}$], and $L/L({\rm H}\beta)$.  It is
written by every run: the H and He recombination lines need only $n_e$,
$T_e$ and the He ionization state, so an H/He model at fixed temperature
gets its table too, with the metal blocks simply absent.  With
...
\code{emis\_h} & Storey \& Hummer (1995) H\,\textsc{i}, case set by
  \code{line\_case} & H$\alpha$ 6565, H$\beta$ 4863, H$\gamma$ 4342,
  H$\delta$ 4103, Pa$\alpha$ 1.876\,$\mu$m, Pa$\beta$ 1.282\,$\mu$m,
  Br$\gamma$ 2.166\,$\mu$m & always; $n_e\,n_{\rm HII}$ \\
\code{emis\_heii} & SH95 He\,\textsc{ii} (case per \code{line\_case}) &
  1640, 3204, 4543, 4687, 5413, 10126 & only where an He\,\textsc{iii}
...
\code{emis\_hei} & Porter et al.\ (2012/2013) He\,\textsc{i}, case-B
  ...
  3890, 4027, 4473, 5877, 6680, 7067, 7283, 10833 & where He\,\textsc{ii}
...
\code{\_dustesc.txt} & (with \code{dust\_emis\_transport}) the infrared spectrum that actually escapes the grid, from the transported packets.  Its ratio to the emitted spectrum in \code{\_dustir.txt}/\code{\_dustsed.txt} is the infrared escape fraction \\
...
\code{\_heimeta.h5} & (with \code{hei\_metastable}) the He\,\textsc{i} 2$^3$S diagnostic: \code{n\_2s3} [cm$^{-3}$] (with the \code{DIST\_CM} and \code{A31} keywords), \code{Phi\_3} [s$^{-1}$], \code{n\_H}, \code{n\_e}, \code{T\_e}, \code{LeafXYZ}, \code{LeafSize} --- read by the Python \code{HeiMetaData} class for column and 10833~\AA\ optical-depth maps, and convertible to a LaRT scattering-medium input by \code{heimeta\_to\_lart.py} \\
...
The \code{\_heimeta} file (\code{hei\_metastable}) is read by the
\code{HeiMetaData} class, which adds a line-of-sight column map and the
10833~\AA\ line-center optical depth:
\begin{verbatim}
...
img, ext = d.tau10833_map(v_turb=0.0,      # tau_0 map (km/s turbulence)
                          component="doublet")  # blended J'=1,2 pair
fig, ax = d.plot_column("n_2s3", log=True)
fig, ax = d.plot_tau10833(component="all")
\end{verbatim}
\code{column\_map} is the same flux-conserving deposit as the line maps
with weight $n\,V$, so $\sum N\,A_{\rm pix} = \sum n\,V$ to machine
precision.  \code{tau10833\_map} deposits the line-center opacity
$\kappa_0 = \frac{\sqrt{\pi}\,e^2}{m_e c} f\,\frac{\lambda}{b}\,n_3$,
$b = \sqrt{2 k T_e/m_{\rm He} + v_{\rm turb}^2}$, so a pixel value is
$\int\!\kappa_0\,{\rm d}l$.  \code{component} selects the resolved
$J'=0$ line (10832.06~\AA), the blended $J'=1,2$ \code{'doublet'}
(10833.22/10833.31~\AA, 0.089~\AA\ apart, inside the thermal Doppler
width), or \code{'all'}; the constants ($\lambda$, $f$, $A$) come from
\code{data/atomic/hei\_metastable.txt}.  The map warns when line-center
$\tau_0 > 1$: the diagnostic assumes optically thin 10833 (line transfer
is out of scope).  The command line takes \code{--column n\_2s3} or
\code{--tau10833 --component doublet --vturb 5}.

Where line transfer \emph{is} wanted (resonant-scattering emission
profiles from the metastable medium), \code{heimeta\_to\_lart.py} (in
\code{tools/python/}) converts a \code{\_heimeta} file into a
scattering-medium input for the resonance-line code LaRT (line
\code{HeI\_10833}): \code{--format car}
(default, uniform grids) writes the all-in-one Cartesian file with
\code{gasDen} $= n_3$, \code{T} $= T_e$, and zero velocities;
\code{--format amr} writes the generic AMR table with $n_3$ in the
\code{n\_ion} column.  \code{--field} selects a different leaf field for
the scatterer.  A worked end-to-end case (converted grid + LaRT namelist)
is in \code{examples/hei\_metastable/lart/}.
...
\code{uni\_dda} & uniform grid: \code{'amr'} walk $\equiv$ octree (bit-identical); DDA walk to rounding \\
...
\code{hei\_meta} & He\,\textsc{i} 2$^3$S metastable: OH18 analytic limits, module $n_3$ vs.\ the Python closed form, $\Phi_3$ from the band tally, and the 10833~\AA\ column/optical-depth diagnostic (conservation + hand integral) \\
\bottomrule
\end{tabular}
\end{center}
...
&parameters
 ...
 par%distance_unit = 'pc'        par%dust_density_law = 'global_dgr'
 par%nnu_ion = 32
 ...
 par%ion_dust_kext = '../../data/kext_albedo_WD_MW_3.1_60_D03.all_2003'
...
\item \textbf{FUV and the PDR}: without \code{add\_fuv} there is no
  ...
  neutral; with it, FUV penetrates the I-front and photoionizes the metal
  stages whose thresholds lie below 13.6\,eV (C\,\textsc{i} 11.3, S\,\textsc{i}
  10.4, Si\,\textsc{i} 8.2, Fe\,\textsc{i} 7.9, Mg\,\textsc{i} 7.6\,eV,
  \ldots), producing the PDR-side C\,\textsc{ii} and Mg\,\textsc{ii}.  In the
  FUV band the gas opacity is exactly this low-threshold metal
  photoionization (\code{ion\_metal\_abs}, on by default; H and He do not
  absorb below 13.6\,eV), so the FUV is attenuated by that photoionization
  together with dust --- only where those metals are already ionized does
  dust extinction alone remain.  The metal stage fractions iterate with this
  opacity self-consistently; the corresponding metal photoheating and the
  grain photoelectric heating are not in the thermal balance, so PDR-zone
...
\item \textbf{kext tables}: the default astrodust and DL07 tables cover the
  EUV directly (astrodust: $1.0\times10^{-4}$--$3.981\times10^4\,\mu$m;
  DL07: $6.2054\times10^{-5}$--$3.981\times10^4\,\mu$m).  If
  \code{ion\_dust\_kext} names an explicit table, that table must cover the
  transported band; tables are sorted on read (the D03 file has an
  out-of-order seam).
