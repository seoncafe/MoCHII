...
\code{ion\_energy\_mode} & 'continuous' & packet-energy transport mode.  The default samples the source energy CDF continuously and evaluates cross sections at the sampled energy.  Set \code{'grouped'} explicitly only for compatibility/reproduction calculations that use one representative energy per bin \\
\code{nnu\_ion} & 16 & diagnostic ionizing-energy bins in \code{'continuous'} mode (gates use 32); changing it does not change the physical solution.  In \code{'grouped'} mode it is the transport/solver bin count \\
\code{eion\_min}, \code{eion\_max} & 13.598, 100 & ionizing band edges [eV] \\
\code{ion\_align\_edges} & T & align diagnostic bin edges to the ionization thresholds.  It does not affect continuous-energy transport.  In \code{'grouped'} mode it also prevents a transport bin from straddling a threshold; segments are log-uniform and low-priority metal edges are dropped safely when necessary.  Set F for a single log grid \\
...
\code{ion\_dust\_kext} & '' & kext table with EUV coverage (e.g.\ \code{data/kext\_albedo\_WD\_MW\_3.1\_60\_D03.all\_2003}); required \\
...
&parameters
 ...
 par%nnu_ion = 32
 par%ion_energy_mode = 'grouped'  ! dust example; continuous mode currently requires ion_add_dust = .false.
