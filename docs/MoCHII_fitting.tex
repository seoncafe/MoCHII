...
products: the cooling-function fits evaluated inside the thermal
iteration, currently 28 ions of the eleven-element registry, and the
$n$-level data files (level energies, $A$-values, and Chebyshev fits of
the effective collision strengths in Burgess--Tully scaled space) that
supply the output-time line emissivities.  Both are verified against the
...
$n_{\rm crit}$, where the cooling fit overestimates the cooling.  This
is now the leading approximation of the thermal loop: on the Cloudy
c25.00 state of the HII40 benchmark ($n_e \simeq 110$~cm$^{-3}$) the fitted
line cooling exceeds the Cloudy inventory by 11.6\%, of which
$+5.1$ points come from [O\,\textsc{iii}] and $+3.9$ from
[C\,\textsc{ii}], while the same ions evaluated with the $n$-level solve
at the local $n_e$ land within 2.4\% and 6\%.  A density-dependent
$\Lam(T, n_e)$ is the natural next step; the emissivities themselves
always come from the $n$-level data.
