...
\MoCHII{} & nearest-neighbor ratio chain & separate & no & optional, damped
fixed point & yes, default on & H/He channels only & outer bisection in
$\log\Te$ \\
\end{longtable}
\endgroup
...
and neither \code{HCTIon} nor \code{HCTRecom} appears in the file.  In
\MoCHII{}, \code{species\_fractions} adds \code{nHII*cx\_rate(cxi)} and
\code{nHI*cx\_rate(cxr)} to the metal rates
(\code{src/species\_\allowbreak mod.f90}:823--827) while \code{solve\_ion\_cell}
(\code{src/ion\_\allowbreak balance\_\allowbreak mod.f90}:72--96) has no charge transfer term at all.

Physically this means the same thing in all five: charge is not conserved in
the hydrogen-plus-metal pair.  Every charge transfer recombination of a metal
ion that these codes count also ionizes a hydrogen atom, and that hydrogen
ionization is never entered.  The magnitude in \MoCHII{} is
\emph{unmeasured}, and should not be quoted as small until it is measured.
It is expected to be negligible in the fully ionized core, where the hydrogen
photoionization rate dominates the balance; it should be expected to matter at
and beyond the ionization front, where $x({\rm H}^0)$ rises and the missing
term is proportional to it.
...
(\code{par\%metal\_ne}, default off), but when it is on it is solved as a
