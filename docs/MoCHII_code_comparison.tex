%% MoCHII against the Monte Carlo photoionization codes closest to it in
%% method: algorithmic and physical comparison.
...
\title{MoCHII Among the Monte Carlo Photoionization Codes:\\
An Algorithmic and Physical Comparison with M$^3$, MAPPINGS V 5.2.1,\\
\textsc{ionize}, \textsc{CMacIonize}, \textsc{Torus}, and \textsc{Mocassin}}
\author{Kwang-Il Seon}
\date{\docmoddate}
\maketitle

\begin{abstract}
This memo compares \MoCHII{} (MOnte Carlo for \HII{} regions) with the
Monte Carlo photoionization codes closest to it in method, and with the
deterministic one-dimensional plasma code two of them descend from.  The
principal pairwise comparison is with \Mthree{} (Messenger Monte Carlo
MAPPINGS V) and its parent \Mappings{}, whose local trees are
\code{\char`~/RT\_Codes/MAPPINGS/M3-main} and
\code{\char`~/RT\_Codes/MAPPINGS/mappings\_V-521}.
Section~\ref{sec:others} adds three further three-dimensional codes:
\Wood{}, the code of Wood, Mathis \& Ercolano (2004) that supplies the WME
column of the Lexington benchmark tables; \CMI{}, which its own source
describes as the C++ version of that code; and \Torus{}, a
radiation-hydrodynamics code whose photoionization loop borrows from both
\Wood{} and \Mocassin{}.  \Mocassin{} is \MoCHII{}'s quantitative validation
reference and appears throughout.  Section~\ref{sec:hlines} compares the one
emission process all of them compute, the hydrogen recombination lines,
together with \Cloudy{} c25.00, which is the one code in reach that solves the
hydrogen atom instead of reading a table.  There are four method classes among
the seven, and no two codes agree on how the case A/B question should even be
posed.

The comparison rests on connected execution paths and quantitative tests
rather than on routine names.  \MoCHII{} is strongest as a modern
three-dimensional transport and synthetic-observation engine: octree AMR, an
analytic absorption estimator, a dedicated Cartesian DDA backend,
solution-driven ionization-front refinement, dust absorption and scattering,
stochastic dust/PAH emission, and observer peel-off imaging; and it is the
only code here whose thermal balance and emergent line list come from the
same level populations at the same electron density.  \Mappings{} is
strongest as a deterministic one-dimensional general plasma engine: a
30-element option, density-dependent multilevel cooling,
finite-source-age photoionization, non-equilibrium cooling, radiation
pressure, and steady radiative MHD shocks with an iterated precursor.  It
also has the most physical hydrogen-line treatment of the six
three-dimensional and one-dimensional nebular codes compared here, a case
parameter computed from the local Lyman-line escape probability rather than
chosen.  \Mthree{} occupies a distinct middle ground: it grafts
three-dimensional Monte Carlo transport onto an older MAPPINGS V
5.1.13-era physics tree, so it combines broad MAPPINGS microphysics with
arbitrary Cartesian geometry, but it does not inherit all later 5.2.1
changes or expose all parent modes through its public 3D driver.

The limitations are correspondingly different.  \MoCHII{} has an
\HII-region-centered ion inventory; \Mthree{} has no AMR, a
communication-heavy uniform-grid packet algorithm, and a 3D driver that
hardcodes pure case B; \Mappings{} is restricted to spherical or
plane-parallel 1D structures and uses an outward-only diffuse field;
\Wood{} is serial, dust-free, and has no convergence test; \CMI{} has no
global convergence criterion, no dust in the photoionization path, and a
temperature solve that is off by default; and \Torus{} has the strongest
transport and parallelism of the family alongside the least complete gas
microphysics, missing \HeII{} photoheating and emitting no helium line at
all.  The recommended strategy is to retain the \MoCHII{} transport
architecture, use \Mappings{} as the current microphysics and 1D
shock/time-dependence reference, use \Mthree{} to study how MAPPINGS physics
behaves after being mapped into three dimensions, and read \Torus{} for
transport and parallel design rather than for gas physics.
\end{abstract}
...
equilibrium temperature, SEDust stochastic emission, transport of the
reradiated infrared back onto the grains, and IR maps --- all from one
grain model, whose own size-integrated extinction table feeds the
transport, so extinction and emission refer to the same grains (gated). & Rich
...
%==============================================================================
\section{The Monte Carlo photoionization codes closest in method}
\label{sec:others}

\Mthree{} is not the only three-dimensional Monte Carlo photoionization code
this comparison has to account for.  Three others are close enough in method
that \MoCHII{}'s choices are best read against them: \Wood{}, the code of
Wood, Mathis \& Ercolano (2004) that supplies the WME column of the Lexington
benchmark tables; \CMI{}, which its own source calls ``the C++ version of
Kenny Wood's photoionization code'' (\code{src/CMacIonize.cpp}:392--393); and
\Torus{}, a radiation-hydrodynamics code whose photoionization loop borrows
from both \Wood{} and \Mocassin{}.  \Mocassin{} itself is \MoCHII{}'s
quantitative validation reference and appears throughout this memo rather
than as a separate subsection.

The evidence standard of Section~\ref{sec:purpose} applies unchanged: every
statement below was read in the source, and where a capability exists in a
tree but is not reached from the photoionization driver it is labeled as
such.  The \emph{cooling} inventories of these same four codes were audited
separately and are not repeated here; that audit, with its file and line
references and the defects it found on each side, is
\code{docs/MoCHII\_cooling\_analysis.pdf}.  This section covers the other
axes.

\subsection{\Wood{}: the reference implementation of the method}

\Wood{} is the ancestor of this family and the most instructive comparison
precisely because it is small: 33 fixed-form Fortran files, one executable,
no libraries.

Packets carry photon number rather than energy, and the band is
13.6--54.4~eV only (\code{planckset.f}:19--21).  The estimator is
Lucy's path-length form, but \emph{frequency-unresolved}: as a packet crosses
a cell, the code accumulates the rate integrals themselves --- one
accumulator for each species it knows about, \code{jmeanH},
\code{jmeanHe}, \code{jmeanN(...,1:3)} and so on
(\code{tauint.f}:184--268) --- and never stores a mean intensity.  There is
no $J_\nu(\nu,\mathrm{cell})$ array anywhere in the code.  The consequence
is structural rather than numerical: a rate integral for any species not in
that hardcoded list cannot be recovered after the run, so adding an ion is a
code operation.  \MoCHII{}'s \code{jt\_ion(nnu,nleaf)} band tally is the
opposite choice, and it is what makes its registry a data operation.

The opacity is hydrogen and helium only
(\code{tauint.f}:159--161); the metals are tallied but never attenuate.  The
diffuse field, by contrast, is fully explicit and physically consistent: the
balance uses the Verner \& Ferland case-A total recombination coefficient
(\code{recomb.f}:13--15, $4.19\times10^{-13}$~cm$^3$\,s$^{-1}$ at
$10^4$~K), and after each absorption the packet is re-emitted as a
ground-state continuum photon with probability $\alpha_1/\alpha_A$, sampled
from a Saha--Milne emissivity distribution, with four helium channels and
\HeI{} Lyman~$\alpha$ handled on the spot
(\code{probset.f}:28--60, \code{mcionize.f}:278--339).

The grid is a uniform Cartesian mesh with $65^3$ compiled in
(\code{grid.txt}:3), and the density is a hardcoded analytic function of
position evaluated on cell centers (\code{density.f}:15--20); there is no
grid-file reader, so a simulation-derived density field cannot be read at
all.  The code is \emph{serial}: no MPI and no OpenMP appear in any source
file.  Seven elements and 21 stages are followed, without He$^{++}$, and
with C$^0$ and S$^0$ assumed absent; $\nelec$ comes from ionized hydrogen and
helium only, which the source states outright (\code{tbal1.f}:39--40).

Two properties of its iteration are worth recording because \MoCHII{} spent
real effort on both.  There is \emph{no convergence test} --- the manual
prescribes running many iterations and comparing successive ones by eye ---
and the ionization is iterated alone for the first four iterations, the
thermal balance starting only at the fifth (\code{itbal.f}:27--41).  The
temperature solve is a logarithmic secant on the gain/loss ratio
(\code{tbal1.f}:78--107); below 4000~K the cell is declared neutral and
pinned at 500~K.

There is no dust.  The only grain-related term is a scalar gas-heating
factor $3\times10^{-25}\,n\,\nelec$ scaled by an input parameter that the
shipped deck sets to zero (\code{ioneng.f}:56--68).  Outputs are a
direction-binned escaping spectrum on a $20\times20$ grid of
$(\theta,\phi)$ and an emissivity cube; there is no imaging, and the manual's
recipe for a map is to sum the cube along an axis.  The sibling directory
\code{diffuse/} in the same distribution is a different code, a
dust-scattering polarization Monte Carlo with Stokes peel-off imaging and no
gas physics; it shares only utility routines and should not be read as
\Wood{}'s imaging stage.

\subsection{\CMI{}: the same method, modernized}

\CMI{} inherits \Wood{}'s physics and rewrites its software.  The transport
is a random optical depth with the Lucy path-length estimator
(\code{src/IonizationPhotonShootJob.hpp}:135,
\code{src/DensityGrid.hpp}:150--197), the diffuse re-emission is the
Wood--Mathis--Ercolano scheme reimplemented term by term
(\code{src/PhysicalDiffuse\-Reemission\-Handler.cpp}:52--203), and the
hydrogen
line coefficients are numerically identical to \Wood{}'s
(Section~\ref{sec:hlines}).  What changed is everything around the physics.

Frequencies are continuous: a packet's cross sections are evaluated once at
emission (\code{src/PhotonSource.cpp}:230--244) and there is no band grid in
transport at all.  This removes the discretization question that
Section~\ref{sec:specdisc} shows to be the largest single lever on \MoCHII{}'s
helium and metal ionization, and replaces it with sampling noise in the rate
integrals.  Three grid types are offered --- uniform Cartesian (the
default), an octree AMR with Morton-style keys, and a Voronoi mesh that can
move with the fluid --- plus a fourth structure, a subgrid decomposition used
by the task-based mode.  Refinement can be static or solution-driven, on
either cell opacity or O\,\textsc{i} content
(\code{src/OpacityAMRRefinementScheme.hpp}:90--99,
\code{src/OIAMRRefinementScheme.hpp}:87--94).  Two gaps are visible in the
source: \code{AMRRefinementScheme::coarsen} is declared and
has no caller anywhere, so the mesh can only ever get finer, and the MPI
branch of the refinement carries the comment \code{///~WE HAVE TO DO SPECIAL
THINGS FOR MPI HERE!} (\code{src/AMRDensityGrid.hpp}:337).

Parallelism is OpenMP through a job market plus MPI as a photon-count
decomposition over a \emph{fully replicated} grid, with the accumulators
reduced across ranks and the cell solve split by contiguous cell block
(\code{src/IonizationSimulation.cpp}:395--397, 458--617).  That is the same
division of labor as \MoCHII{}'s, which also replicates the grid and
distributes packets; \Torus{} is the one code here that decomposes the grid
itself.  MPI is refused at startup for the dust, hydrodynamic,
emissivity and task-based modes.

Three properties bear directly on the comparisons this memo makes elsewhere.
The metal line emissivities are solved from level populations at the local
electron density (\code{src/LineCoolingData.cpp}:1785--1798), which is the
axis \MoCHII{} closed with \code{par\%cooling\_model = 'local\_ne'}.  The
temperature solve is \emph{off by default} and, when on, is capped at
30\,000~K for a stated reason --- ``helium charge transfer rates are only
valid until 30,000~K'' (\code{src/TemperatureCalculator.cpp}:831--833).  And
there is \emph{no global convergence criterion of any kind}: the iteration is
\code{while (loop < \_number\_of\_iterations)}
(\code{src/IonizationSimulation.cpp}:359), with a plain input count, no
cell-maximum test, no integrated measure and no early exit.  Metals carry
neither opacity nor free electrons, there is no collisional ionization
anywhere in the tree, and He$^{++}$ does not exist as a species.

Dust is absent from the photoionization path --- a case-insensitive search of
the ionization, temperature and state-calculator sources returns nothing.  It
exists instead as a separate hardwired mode whose own documentation calls it
a first-year lab project (\code{src/CMacIonize.cpp}:50--53), with grey
constants in each band, a forced first interaction, single-observer peel-off
and Stokes imaging.  There is no dust temperature and no infrared emission
anywhere in the tree.

Synthetic observations are the weakest axis.  The ionization mode offers
point ``trackers'' that record the absorption or the spectrum inside a named
cell, and grid dumps whose field list contains no emission-line entries.  A
line-of-sight emission integrator, \code{get\_total\_emission}, is
implemented for all three grid types and \emph{has no caller}; the
emissivities it would integrate are themselves produced only by a separate
post-processing mode, which is Cartesian-only and rejects MPI.  Two
statements in the tree's own documentation are stale and should not be quoted
from it: the claim that the code is parallelized with OpenMP only
(\code{src/CMacIonize.cpp}:416--420), contradicted by the working MPI code,
and a comment implying a Python library replaced the in-run emissivity
calculation when the compiled mode is the fuller route.

\subsection{\Torus{}: photoionization inside a radiation-hydrodynamics code}

\Torus{} is the most capable of the three in transport and the least
specialized to photoionization, which shows in both directions.

The estimator is again Lucy's, with the path length weighted by
$\ell w/(h\nu)$ and accumulated for each ion
(\code{photoionAMR\_mod.F90}:6679--6692); a modified random walk handles the
optically thick regime.  Dust scattering uses real Mie phase matrices rather
than a Henyey--Greenstein approximation (\code{:3283}).  The diffuse field is
transported by default, and the switch that turns it off removes only the
ionizing continuum from the redraw spectrum (\code{:3347--3349}), leaving the
sub-Lyman lines and the dust continuum in place --- so ``on the spot'' means
something narrower here than in \MoCHII{}.  Independently of that, a
\code{caseB} flag replaces the hydrogen recombination coefficient with the
\emph{constant} $2.7\times10^{-13}$~cm$^3$\,s$^{-1}$, with no temperature
dependence (\code{photoion\_utils\_mod.F90}:305--310); setup only warns if it
is combined with a transported diffuse field.  This is exactly the conflation
\MoCHII{} separated into \code{par\%case\_ab} and \code{par\%line\_case}, left
here as a warning rather than as two parameters.

The frequency grid is 1000 logarithmic bins from 1~mm to about 124~eV with
four extra points pinned at $\pm1\%$ around the helium edges
(\code{:2124--2151}) --- threshold \emph{bracketing}, the same concern as
\MoCHII{}'s threshold alignment, addressed by refining around the edge rather
than by placing a bin boundary on it.

The grid is an AMR tree that can be one-, two- or three-dimensional (binary,
quad, octree), with a cylindrical option and a two-dimensional Cartesian slab
whose missing third dimension is compensated by re-weighting the emission
direction.  Solution-driven refinement is live and physically the same
criterion \MoCHII{} uses: a relative neutral-hydrogen-fraction jump across
each face against a tolerance
(\code{hydrodynamics\_mod.F90}:14582--14618).  One switch is dead and should
not be cited as a capability: \code{doPhotoRefine} is read from the input file
and its only use site is \code{if (doPhotoRefine .and. 0 == 1)}
(\code{:4428}).

Parallelism is the most developed of the four: MPI decomposes the grid
itself, with packets shipped between ranks in batched stacks, plus extra
ranks assigned as duplicates of busy domains apportioned by measured packet
crossings (\code{loadbalance\_mod.F90}:16--219), and OpenMP threads the
equilibrium solve over cells (\code{:3855--3864}).  \MoCHII{} distributes only
transport and keeps one grid copy per node; \Torus{} distributes the state.

Convergence is a nested pair: an inner alternation of ionization and thermal
solves until both change by less than $1\%$ in every subcell, and an outer
test on the \emph{cell-maximum} fractional temperature change below $0.05$
(\code{:4324--4339}).  There is no volume-integrated measure.  What \Torus{}
does instead, when the outer test stalls, is diagnose the cause: cells the
Monte Carlo has undersampled are counted, and finding any of them doubles the
packet count and continues (\code{:4344--4347}).  That is an answer to the
same noise-floor problem \MoCHII{} solved with \code{par\%conv\_crit =
'vol'}, from the other end --- raise the photon budget until the cell-maximum
test becomes meaningful, rather than change the measure.

The heating inventory is exactly three terms: \HI{} photoionization, \HeI{}
photoionization, and dust (\code{:7810--7823}).  \HeII{} photoheating is
absent although He$^{++}$ is tracked, and there is no metal photoheating and
no grain photoelectric heating in this balance.  Recombination data are
Verner \& Ferland for H and He and Pequignot et al.\ (1991) plus Nussbaumer
\& Storey (1983) for the metals --- \emph{the \Mocassin{} set}, so the
in-code swap experiment reported in Section~\ref{sec:recombdata} transfers to
\Torus{} unchanged.  Its H$^+$ and He$^{++}$ coefficients agree with
\MoCHII{}'s default $\alpha_A$ to four digits; only He$^+ + e$ differs, and
there \Torus{} sits $4.7\%$ \emph{above} \MoCHII{}, the opposite side from
\Mocassin{}'s low value.  One provenance curiosity: the free-bound $\gamma$
tables \Torus{} reads are byte-identical to \Mocassin{}'s and \MoCHII{}'s
copies, but its own comment attributes them to Ferland (1980) where the other
two carry the Ercolano \& Storey compact format.  The numbers are not in
question, only the citation.

Dust is coupled to the gas in both directions, which no other code in this
comparison does: grain emission enters the gas cooling side and grain heating
the gas heating side of the same balance, or, decoupled, a gas--grain
collisional exchange term carries the energy between them
(\code{:6604--6628}, \code{:6253--6271}).  The infrared emission is a single
equilibrium temperature per cell; there is no stochastic single-photon
heating and no PAH channel on this path, so \MoCHII{}'s SEDust coupling and
its $x_{\mathrm{HII}}$-weighted PAH share have no counterpart.

Imaging is genuine peel-off with the full Stokes vector and $e^{-\tau}$ to
the boundary, cloned at emission and after every scattering
(\code{:9109--9113}, \code{image\_mod.F90}:187--188), for one of four image
types (free-free, forbidden line, recombination line, dust).  It is
monochromatic: the photoionization image path carries a fixed wavelength and
applies no Doppler shift.  \Torus{} does have a velocity-resolved image
adder, but it belongs to the atomic and molecular line transfer modes and is
not wired to the photoionization loop --- present in the tree, not connected.
There is no emergent SED and no datacube from this loop.

\subsection{What the four have in common, and where \MoCHII{} sits}

\begingroup\small\setlength{\tabcolsep}{4pt}
\begin{longtable}{P{0.135\textwidth}P{0.185\textwidth}P{0.185\textwidth}P{0.185\textwidth}P{0.185\textwidth}}
\caption{The Monte Carlo photoionization codes closest to \MoCHII{} in
method, on the axes not covered by the cooling audit.  Entries are what the
photoionization path does, not what exists in the tree.}
\label{tab:others}\\
\toprule
Topic & \Wood{} & \CMI{} & \Torus{} & \MoCHII{} \\
\midrule
\endfirsthead
\multicolumn{5}{l}{\small\itshape Table~\ref{tab:others} continued.}\\
\toprule
Topic & \Wood{} & \CMI{} & \Torus{} & \MoCHII{} \\
\midrule
\endhead
\bottomrule
\endfoot
Estimator & Lucy path length, rate integrals accumulated directly; no
$J_\nu$ stored & Lucy path length, continuous frequency & Lucy path length,
1000-bin grid & Analytic edge walk without scattering, band-resolved
$J_\nu$ tally \\

Grid & Uniform Cartesian, size compiled in; analytic density only & Cartesian
/ octree AMR / Voronoi / subgrid & AMR in 1D, 2D or 3D, cylindrical option &
Octree AMR or raster Cartesian with a DDA walk \\

Solution-driven refinement & none & opacity or O\,\textsc{i} criterion;
no coarsening & $x_{\rm HI}$ gradient across faces & $x_{\rm HI}$ front flags
plus face-neighbor jump \\

Parallel model & serial & OpenMP + MPI over packets, grid replicated &
MPI over the grid with load-balancing ranks, OpenMP over cells & MPI over
packets, shared-memory grid per node \\

Diffuse field & explicit, case A, four He channels & the same scheme,
opt-in, default off & transported by default; \code{caseB} is a constant
coefficient & explicit H/He channels plus the \HeI{} excited channel; case-B
OTS available \\

Global convergence & none (compare iterations by eye) & none (fixed
iteration count) & cell-maximum $\Delta\Te$, packet doubling on undersampling
& cell-maximum or volume-integrated, tolerance-gated \\

Metals & tallied, no opacity, no electrons & tallied, no opacity, no
electrons; no collisional ionization & 22 ions, no opacity, electrons
counted, charge exchange data for five ions only, no collisional ionization &
registry with opacity, optional electrons and heating, two
collisional-ionization models \\

Dust & none & none on this path & absorption, Mie scattering, equilibrium
$T_{\rm dust}$, gas--grain exchange & absorption, scattering, live survival,
$T_{\rm dust}$, stochastic PAH emission \\

Images & escaping spectrum by direction bin; emissivity cube & trackers only;
the line-of-sight integrator is never called & Stokes peel-off,
monochromatic, four image types & multi-observer peel-off, direct/scattered
split, bin-resolved cubes, flux-conserving line maps \\
\end{longtable}
\endgroup

Read across that table, the family divides on one line.  \Wood{} and \CMI{}
are photoionization codes in which the radiation field is a means to the
ionization state, and their synthetic-observation layer is correspondingly
thin.  \Torus{} is a transport code with photoionization added, which is why
its imaging and parallelism are the strongest here and its gas microphysics
the most incomplete --- missing \HeII{} photoheating and, as
Section~\ref{sec:hlines} shows, missing helium lines entirely.  \MoCHII{}
sits with \Torus{} on transport and observation and with \Mocassin{} on gas
microphysics, and it is the only one of the five whose thermal balance and
emergent line list are computed from the same level populations at the same
electron density.  How each of them actually solves the ionization state, and
where the metals enter it, is Section~\ref{sec:ionsolve}.
...
\subsection{How the ionization system is actually solved: no code couples
the elements}
\label{sec:ionsolve}

Comparisons of this family normally stop at the atomic data and the thermal
solver, and leave the structure of the ionization solve as an assumption:
that the established codes solve the ionization of every element as one
coupled nonlinear system, and that \MoCHII{}'s element-by-element cascade is
a reduction of that.  Five codes were read in their own sources to settle it
--- \Mocassin{}, \CMI{}, \Torus{}, \Wood{}, and \Cloudy{} c25.00 --- and
\emph{none of them does}.  Every one solves one element at a time inside
nested outer iterations, and four of the five use the same nearest-neighbor
ratio chain \MoCHII{} uses.  The electron temperature is an outer root find
in all five; it is never an unknown of the ionization system.  The ratio
chain is therefore the standard method of the field, not a
\MoCHII{}-specific approximation.  \Cloudy{} departs from it on exactly two
axes, both identified below, and neither of them is element coupling.
\Mappings{} and \Mthree{} were not part of this audit, and no claim is made
here about their solve structure.

In \Mocassin{}, \code{ionBalance} (\code{update\_\allowbreak mod.f90}:1321) forms the
adjacent-stage ratio \code{ionRatio(elem,ion)}~$=X_{i+1}/X_i$ (declared
\code{:1356}, formed \code{:1495--1499}), accumulates it into cumulative
products (\code{:1506--1517}), and closes each element with a single
normalization scalar, \code{denominator(elem)}, dimensioned
\code{(nElements)} (\code{:1348--1349}, formed \code{:1520--1527}, fractions
\code{:1537--1544}).  There is no ionization matrix in the tree at all: the
only LU factorization, \code{luSlv}/\code{lured}
(\code{emission\_\allowbreak mod.f90}:2126--2164), is called from \code{:2091} inside
\code{equilibrium} for the bound-level populations of the forbidden lines.
The element solves are tied together only by an outer fixed-point recursion,
\code{nIterateX} with \code{maxIterateX = 25} (\code{update\_\allowbreak mod.f90}:84--85,
recursive self-call \code{:1608}).  \CMI{} writes the same chain out by hand,
one \code{\#ifdef} block for each element:
\code{compute\_\allowbreak ionization\_\allowbreak states\_\allowbreak metals}
(\code{Ionization\allowbreak State\allowbreak Calculator.cpp}:323--501, the scheme documented at
\code{:280--310}), with carbon at \code{:392--403} as
\code{C21 = jCp1/(ne*alphaC[0])}, \code{C31 = C32*C21},
\code{sumC\_inv = 1./(1.\,+\,C21\,+\,C31)}, and independent blocks for C
(\code{:389--404}), N (\code{:406--434}), O (\code{:436--455}), Ne
(\code{:457--471}) and S (\code{:473--500}); the metals are solved only after
H/He has been frozen (\code{:116--185}).  \Torus{} blocks its flat ion list
by nuclear charge (\code{photoion\allowbreak AMR\_\allowbreak mod.F90}:6859--6867), forms the same
ratio (\code{:6880--6881}), chains it (\code{:6891}), renormalizes each
element (\code{:6902--6904}), and under-relaxes the update by 0.6
(\code{:6853}, \code{:6914}).  \Wood{} is the extreme case: the chain is
written out for carbon at \code{ionize2.f}:68--77 and as three links for
nitrogen and sulfur at \code{:96--98}, each metal block reads only \code{ne},
\code{h0} and \code{he0} from the already-converged H/He solution and never
another metal's state, and there is no iteration on the metal fractions at
all --- one pass for each cell in each Monte Carlo iteration.  Its only
matrix, \code{SIMQ} in \code{SIMQ2.f}, again belongs to the five-level bound
populations of the cooling function (\code{linecool1.f}:110).  \MoCHII{} does
exactly what those four do: \code{species\_fractions}
(\code{src/species\_\allowbreak mod.f90}:810--840) takes $\Te$, $\nelec$, $n_{\rm HI}$
and $n_{\rm HII}$ as inputs and returns the chain product for one element.

\begingroup\footnotesize\setlength{\tabcolsep}{3pt}
\begin{longtable}{P{0.13\textwidth}P{0.145\textwidth}P{0.08\textwidth}P{0.09\textwidth}P{0.09\textwidth}P{0.08\textwidth}P{0.115\textwidth}P{0.11\textwidth}}
\caption{How the ionization system is solved in the five codes read in source
for this subsection, with \MoCHII{} as the last row.  Entries describe the
connected photoionization path, not what exists in the tree.  ``not audited''
means the axis was not read for that code and no claim is made about it.}
\label{tab:ionsolve}\\
\toprule
Code & Ionization solve & Elements & Two-way charge exchange & Metals in
$\nelec$ & Metal opacity & Metal diffuse & $\Te$ solve \\
\midrule
\endfirsthead
\multicolumn{8}{l}{\small\itshape Table~\ref{tab:ionsolve} continued.}\\
\toprule
Code & Ionization solve & Elements & Two-way charge exchange & Metals in
$\nelec$ & Metal opacity & Metal diffuse & $\Te$ solve \\
\midrule
...
\bottomrule
\endfoot
\Mocassin{} & nearest-neighbor ratio chain & separate & no & yes,
charge-weighted & yes, all shells & continua trans\-ported, lines discarded &
outer secant, 25 passes \\

\CMI{} & chain, hand-written for each element & separate, metals after H/He &
no & no & no & no metal channel & outer three-point power law, off by
default \\

\Torus{} & chain, under-relaxed 0.6 & separate & no & yes, weight $Z-N$ &
no; H I and He I only & not audited & outer Brent, under-relaxed \\

\Wood{} & chain, one pass, no metal iteration & separate, metals after H/He &
no & no & no & not audited & outer secant in $\log$, from iteration 5 \\

\Cloudy{} c25.00 & $N\times N$ LU with iterative refinement & separate & yes,
into hydrogen's own rows & yes, summed then Brent root find & yes, all shells
& continua and lines, both re-ionize & outer Brent inside the pressure
loop \\

\MoCHII{} & chain, but solved for its root at each $\nelec$ rather than
substituted into & separate & \textbf{yes} & yes, default on, damped fixed
point & yes, default on & H/He channels only & outer bisection in
$\log\Te$ \\
\end{longtable}
\endgroup

\Cloudy{}'s first difference is the matrix, and its reason is physical rather
than numerical.  It allocates a square matrix \code{xmat} of order
\code{ion\_range = IonHigh-IonLow+1} (\code{ion\_solver.cpp}:70--72),
assembles it in \code{fill\_array()} (\code{:538--748}), and factorizes it at
\code{:233} through \code{newton\_step.cpp}:346--400
(\code{getrf\_wrapper}/\code{getrs\_wrapper}, with \code{nrefine = 3} rounds
of iterative refinement), one row being replaced by abundance conservation
(\code{:883--889}).  The matrix is deliberately not tridiagonal: the
ionization block at \code{:727--736} runs the destination stage from
\code{ion\_from+1} up to \code{IonHigh} and fills arbitrary super-diagonal
entries, because Auger multi-electron ejection removes several electrons in
one event and therefore couples stage $i$ to $i+2,\,i+3,\dots$
(\code{:661--668}).  A nearest-neighbor chain cannot represent that, which is
exactly why the \MoCHII{} hard-spectrum plan of Section~\ref{sec:hardplan}
replaces the chain with a small $n_{\rm stage}\times n_{\rm stage}$ matrix in
its HS6 increment, at the point where an inner-shell channel opens.  Two
consequences are worth recording.  The legacy chain solver is still in the
file --- \code{solveions()} at \code{:1317--1364}, with the tridiagonal
matrix drawn in the comment at \code{:1290--1315} --- and it is dead code,
never called in c25.  And the matrix buys \Cloudy{} nothing in element
coupling: \code{ConvBase} loops \code{nelem} from \code{ipHYDROGEN} to
\code{LIMELM} calling \code{ion\_wrapper(nelem)}
(\code{conv\_base.cpp}:557--566), one solve for each element, inside an inner
repeat loop of \code{nconv}~$=3$ passes (\code{:475}, \code{:539}).

The second difference is the one that matters physically for the codes
compared here: \Cloudy{}'s charge transfer is two-way in the bookkeeping.  A
reaction $X^{i+1}+{\rm H}^0\rightarrow X^{i}+{\rm H}^{+}$ recombines the
metal and ionizes the hydrogen in the same event, so it is a source and a
sink on both sides.  \code{iso\_\allowbreak charge\_\allowbreak transfer\_\allowbreak update()}
(\code{iso\_\allowbreak ionize\_\allowbreak recombine.cpp}:18--50), called at the top of every ion
solve (\code{ion\_solver.cpp}:68), accumulates hydrogen's charge transfer
ionization and recombination totals over all heavier elements and all their
stages, each weighted by the partner ion density (\code{:38--45}), with the
source comments naming the symmetry outright (``charge transfer ionization of
nelem1, recombination for other species'').  Those totals then enter
hydrogen's own matrix rows: \code{ction} at \code{ion\_solver.cpp}:687 with
its off-diagonal pair at \code{:696--697}, and \code{ctrec} at \code{:709}
with \code{:723--724}; the H-like level matrix carries the same terms under
\code{if(nelem==ipHYDROGEN)} (\code{iso\_level.cpp}:121--124).  Both sides
read the \emph{same} \code{Char\allowbreak Exc\allowbreak Ion\allowbreak Of}/\code{Char\allowbreak Exc\allowbreak Rec\allowbreak To} arrays, and
\code{ChargTranEval()} is called once in each outer loop precisely so that
they do, as its own comment states: ``charge transfer evaluation needs to be
here so that same rate coefficient used for H ion and other recombination''
(\code{conv\_base.cpp}:498--502).  \Cloudy{} then \emph{checks} the balance
instead of assuming it: \code{:578--583} forms the net rate
\code{netion[nlo][nhi]} as the difference of the two directions, and a
failure declares the zone unconverged with the reason ``\code{CX
inconsistency}'' (\code{:611--620}).  The honest caveat is that within a
single LU the partner's density is a frozen coefficient, so exact charge
conservation is reached by the outer iteration together with that check, not
inside one matrix solve.

The other four codes are all one-way in that bookkeeping, and so is
\MoCHII{}.  The evidence is specific in each case.  In \Mocassin{} the
four-parameter table \code{chex(nElements,\allowbreak nstages,4)}
(\code{update\_\allowbreak mod.f90}:1358--1359) is populated for He through Fe at
\code{:1392--1451}, fitted at \code{:1462--1463}, hard-zeroed outside
6000--50\,000~K at \code{:1466}, and enters the \emph{metal} denominator as
\code{chex(elem,ion,1)\allowbreak *Hden*ionDen(H0)} (\code{:1498--1499}) --- but
\code{chex(1,\dots)} is never assigned anywhere in the tree, so it keeps the
zero it was initialized with at \code{:1392}.  Hydrogen's equation is the
same generic line \code{:1495--1499} evaluated at \code{elem=1, ion=1}, which
reduces to photoionization plus collisional ionization over
$\nelec\alpha_{\rm tot}$ and contains no charge transfer term.  Helium does
carry one against H$^0$ (\code{chex(2,1,:)}, \code{chex(2,2,:)}, set
\code{:1395--1396}), but one-way, and with \code{deltaE\_k(2,*) = 0} it has no
reverse rate; the reverse rate is nonzero only for N I and O I, the only two
entries ever set (\code{deltaE\_k(7,1) = 10863.},
\code{deltaE\_k(8,1) = 2205.} at \code{:1448--1449}, gated \code{:1475}).  In
\CMI{}, a search for \code{charge\_transfer} returns no hit above line 501 of
\code{Ionization\allowbreak State\allowbreak Calculator.cpp}, that is, none inside the H/He solve
(\code{:649--753}), whose only term beyond photoionization and radiative
recombination is the helium Lyman-$\alpha$ on-the-spot term \code{ch2}
(\code{:672--674}, used \code{:712}); \code{Charge\allowbreak Transfer\allowbreak Rates.cpp}:49--52
and \code{:196--199} even \code{cmac\_error()} on a request for hydrogen with
itself.  In \Torus{}, \code{get\allowbreak Charge\allowbreak Exchange\allowbreak Recomb} holds data for N IV,
O II and O III only (\code{photoion\_\allowbreak utils\_\allowbreak mod.F90}:840--868) and
\code{get\allowbreak Charge\allowbreak Exchange\allowbreak Ion} for N I and O I only (\code{:885--906}); both
initialize to zero and fall through for every other nuclear charge
(\code{:838}, \code{:883}), and the hydrogen balance calls those same
routines with \code{parentIon = grid\%ion(2)} and \code{grid\%ion(1)}
(\code{photoion\allowbreak AMR\_\allowbreak mod.F90}:6872--6878), which return zero, so hydrogen
reduces to photoionization over recombination (\code{:6880--6881}).  In
\Wood{}, \code{ct1.f} is the Kingdon \& Ferland package itself
(\code{HCTRecom} at \code{:3}, \code{HCTIon} at \code{:37},
\code{block data ctdata} at \code{:72}, covering $Z = 2$--30) and it is
called in the metal denominators (O \code{ionize2.f}:136--138, N
\code{:83--85}), but \code{h0find.f} has no charge transfer argument in its
interface (\code{:1}), its stated equation set (\code{:6--13}) contains only
hydrogen photoionization, recombination and the helium 2s--2p resonance term,
and neither \code{HCTIon} nor \code{HCTRecom} appears in the file.  \MoCHII{} shared this
until 2026-07-31: \code{species\_fractions} added the two terms to the metal
rates while \code{solve\_ion\_cell} carried none.

Physically the omission means the same thing in all four remaining codes:
charge is not conserved in the hydrogen-plus-metal pair.  Every charge transfer
recombination of a metal ion that these codes count also ionizes a hydrogen
atom, and that hydrogen ionization is never entered.

\MoCHII{} now enters it (\code{par\%charge\_exchange = 'two\_way'}, the
default), so on this axis it sits with \textsc{Cloudy} rather than with the
Monte Carlo codes; \code{'metal\_only'} reproduces the earlier bookkeeping bit
for bit.  The correction turns out to be small on the Lexington benchmarks, and
the reason is structural rather than one of abundance: where the near-resonant
$\mathrm{O}^0+\mathrm{H}^+$ pair locks oxygen to hydrogen the two terms cancel
identically in hydrogen's balance, so only the departure from that lock
survives.  Measured with a single switch changed,
$\langle T[N_{\rm p}N_{\rm e}]\rangle$ moves $-1.41$~K at HII40 and $-0.13$~K
at HII20 and every front by under $0.001$~pc --- inside the tolerance those runs
were converged to by factors of 58 and 533.  The same near-cancellation makes
the substitution map marginally stable in partially neutral gas, which is why
\MoCHII{} solves hydrogen's balance for its root there rather than substituting
into it.

On two axes \MoCHII{} is ahead of three of the four chain codes.  The first is
opacity.  \MoCHII{} attenuates the ionizing field with the metal cross
sections by default (\code{par\%ion\_metal\_abs = .true.}); \CMI{}, \Torus{}
and \Wood{} do not.  \CMI{}'s \code{get\_optical\_depth} returns
$ds\,n\,[\sigma({\rm H^0})x({\rm H^0})+\sigma_{\rm He,corr}x({\rm He^0})]$
and nothing else (\code{DensityGrid.hpp}:117--140), with the task-based path
identical (\code{Density\allowbreak SubGrid.hpp}:557--583), while mean-intensity integrals
are nevertheless accumulated for every ion including the metals
(\code{DensityGrid.hpp}:162--181).  \Torus{} calls \code{phfit2} exactly
twice, for H I and He I --- not even He II
(\code{amr\_mod.F90}:16301--16309) --- and forms
\code{kappaAbsGas = (kappaH + kappaHe)} (\code{:16311}), which is what sets
the packet's optical depth in the cell
(\code{photoion\allowbreak AMR\_\allowbreak mod.F90}:4762, \code{:4741--4743}); its metal cross
sections (\code{ion\_mod.F90}:172) are consumed only to build
\code{photoIonCoeff} (\code{photoion\allowbreak AMR\_\allowbreak mod.F90}:6688--6692).  \Wood{}
carries only $n_{\rm HI}\sigma_{\rm H}+A_{\rm He}n_{\rm HeI}\sigma_{\rm He}$
in \code{tauint.f}:159--161 and in the sub-cell step-back at \code{:169--171},
while computing the Verner metal cross sections at every frequency
(\code{opacset.f}:20--55) and accumulating them into the metal
photoionization rates in the same loop (\code{tauint.f}:194--216).  In all
three the metal photoionizations are counted as rates while the photons are
never removed from the beam: they are sinks that consume no ionizing budget.
\Mocassin{} and \Cloudy{} do remove them.  \code{addOpacity}
(\code{ionization\_\allowbreak mod.f90}:349, called \code{:127} from
\code{ionizationDriver} and driven cell by cell from
\code{iteration\_\allowbreak mod.f90}:142) loops all metals and all shells
(\code{:409--411}) into \code{putOpacity} (\code{:430--439}, \code{:476}),
and that \code{grid\%opacity} is what transport reads
(\code{photon\_\allowbreak mod.f90}:1390, \code{:1442}); \Cloudy{} does the same through
\code{opacity\_\allowbreak addtotal.cpp}:284--291 and
\code{opacity\_\allowbreak add1element.cpp}:41--62 into \code{opac.opacity\_abs[]}, which
attenuates the beam in \code{rt\_continuum.cpp}:182--197.

The size of that omission was measured for this memo and it is not small.
Switching \code{par\%ion\_\allowbreak metal\_\allowbreak abs} off at HII40 moves
$r({\rm He^0}=0.5)$ inward by 0.615~pc: the stages with thresholds between 21
and 40~eV (Ne I 21.56, S II 23.33, C II 24.38, N II 29.60, S III 34.83,
O II 35.12~eV) preferentially remove the softest helium-ionizing photons,
while $\sigma(\HeI{})/\sigma(\HI{})$ rises from 6.1 at 25~eV to 19.9 at
95~eV.  The response is asymmetric and saturates upward: 0.615~pc for a unit
of metal opacity removed against 0.103~pc for a unit added.

The second axis is $\nelec$.  \CMI{} and \Wood{} do not count metal electrons
at all: \code{ne = ntot*(1.-h0\allowbreak +AHe*(1.-he0))}
(\code{Ionization\allowbreak State\allowbreak Calculator.cpp}:134, identically
\code{Temperature\allowbreak Calculator.cpp}:275, whose comment at \code{:273--274} says
``we neglect free electrons coming from ionization of coolants'', the same
assumption being documented at
\code{Ionization\allowbreak State\allowbreak Calculator.cpp}:576--586), and
\code{ne=n*(1.-h0\allowbreak +AHe*(1.-he0))} (\code{ioneng.f}:45, also
\code{ionize2.f}:54), which \Wood{} states as a deliberate approximation at
\code{tbal1.f}:39--41.  \Mocassin{} sums the charge-weighted contribution of
every switched-on element (\code{update\_\allowbreak mod.f90}:1564--1577, written back
\code{:1592--1594}), with the caveats that ions below a fraction of
$10^{-10}$ are silently dropped and stages above \code{nstages} cannot
contribute at all; \Torus{} does the same with weight $Z-N$
(\code{ion\_mod.F90}:1418--1421) and recomputes it after every ionization
update (\code{photoion\allowbreak AMR\_\allowbreak mod.F90}:6916, \code{:5941}).  In \Cloudy{},
$\nelec$ is not an unknown of the matrix either: \code{eden\_sum.cpp}:51--62
builds \code{dense.EdenTrue} as a diagnostic sum over all 30 elements plus
deuterium, molecular and grain electrons (\code{:74}, \code{:78}), and the
electron density actually used changes in exactly one place,
\code{EdenError()} (\code{conv\_\allowbreak eden\_\allowbreak ioniz.cpp}:271--276), as the residual of
a van Wijngaarden--Dekker--Brent root find (\code{:44--150}) each of whose
residuals re-solves the ionization of every element through
\code{ConvIoniz()} (\code{:283--288}).

\MoCHII{}'s metal contribution to $\nelec$ is optional
(\code{par\%metal\_ne}, on by default since 2026-07-31), and it is solved as a
scalar fixed point rather than evaluated once:
\code{charge\_\allowbreak neutrality\_\allowbreak ne} (\code{src/ion\_\allowbreak balance\_\allowbreak mod.f90}:98--123)
solves
\begin{equation}
 \nelec = \nelec({\rm H,He}) + n_{\rm H}\sum_{\rm el} A_{\rm el}
          \sum_i (i-1)\,f_i(\nelec)
 \label{eq:metalne}
\end{equation}
with the same damped iteration and tolerance as the H/He closure.  The reason
is recorded at \code{:105--108} and is worth restating, because the cheaper
choice is wrong for a specific physical reason rather than merely
inaccurate: evaluating the metal sum once at the H/He-only electron density
is the $\nelec\to0$ limit beyond the front, where the H/He term vanishes,
nothing recombines, and every low-ionization-potential metal comes out fully
ionized.

On one axis \MoCHII{} is behind.  Metal recombination continua are not
transported --- \code{src/diffuse\_\allowbreak mod.f90} contains no metal channel.
Measured for this memo they are $0.29\%$ of diffuse channel~2, so the
approximation is justified, but it is an approximation and belongs in the
record as one, because two codes here do not make it.  \Mocassin{} builds
\code{gammaHeavies} in \code{fb\_ff} over \code{do elem = 3, 19}
(\code{emission\_\allowbreak mod.f90}:474, its own comment at \code{:472--473} noting
that $Z>19$ is neglected) from the Milne relation with the true subshell
cross section (\code{:519--521}), folds it into the continuum
(\code{:126--128}) and into \code{sumDiffuseHI} (\code{:983}) and the
cumulative diffuse PDF \code{recPDF} (\code{:1175--1196}), which
\code{new\allowbreak Photon\allowbreak Packet} samples (\code{photon\_\allowbreak mod.f90}:958) and propagates
(\code{:461--466}).  \Cloudy{} goes further and feeds both continua and lines
back into the field, where they genuinely re-ionize: metal recombination
continua are emitted as diffuse radiation from the stored Milne-inverted
metal cross sections (\code{rt\_diffuse.cpp}:196--270), metal recombination
Lyman-$\alpha$ and Balmer photons enter the on-the-spot field
(\code{rt\_ots.cpp}:362--406), both are added into \code{rfield.SummedCon}
(\code{:588--592}), and \code{SummedCon} is exactly what
\code{GammaK}/\code{GammaBn} integrate against the cross sections
(\code{cont\_\allowbreak gammas.cpp}:125, \code{:143}, \code{:409}) to produce
\code{ionbal.Photo\allowbreak Rate\_\allowbreak Shell} (\code{ion\_photo.cpp}:100--110) --- feedback
onto hydrogen included.

Finally, eight properties of the reference codes surfaced in this audit that
change what a comparison against them means, and that a reader using them as
validation references has to know about:

\begin{itemize}[leftmargin=2em]
\item \Mocassin{} substitutes \emph{oxygen's} dielectronic recombination rate
  wherever an element's own rate is missing:
  \code{if (diRec(elem,ion) == 0.)\allowbreak   diRec(elem,ion) = diRec(8,ion)}
  (\code{update\_\allowbreak mod.f90}:1648).
\item \Mocassin{}'s ionization convergence test examines H$^0$, He$^0$ and
  He$^+$ only (\code{:1598--1604}, \code{limit = 0.01} at \code{:1337}).  No
  metal ion fraction is ever tested, so a converged \Mocassin{} zone carries
  no statement at all about its metal ionization.
\item \Mocassin{} discards its metal line photons in production.  The choice
  between a line and the continuum is made (\code{photon\_\allowbreak mod.f90}:925), but
  the line identity is thrown away with \code{nuP = 0} outside \code{lgDebug}
  (\code{:926--941}) and the packet is dumped into \code{linePackets} and
  terminated without transport (\code{:468--475}).  The line PDF is built
  only under \code{lgDebug} (\code{emission\_\allowbreak mod.f90}:1209, metals
  \code{:1251--1289}), \code{grid\%Jdif} is accumulated only under
  \code{lgDebug} (\code{photon\_\allowbreak mod.f90}:1491--1493, \code{:1730--1732})
  with everything otherwise landing in \code{Jste} (\code{:1488--1489},
  \code{:1727--1728}), and \code{nPhotoDif} enters the ion balance only in
  the \code{lgDebug} branch (\code{update\_\allowbreak mod.f90}:1489 versus
  \code{:1495}).  \code{lgDebug} defaults to \code{.false.}
  (\code{set\_\allowbreak input\_\allowbreak mod.f90}:44).
\item \Mocassin{} truncates every element at \code{nstages = 7}
  (\code{set\_\allowbreak input\_\allowbreak mod.f90}:82) while carrying \code{nElements = 30}
  (\code{constants\_\allowbreak mod.f90}:54), so iron stops at Fe VII.
\item \Torus{} creates all four of its ``Ar I--IV'' ions with $n = 18$: all
  four are neutral argon (\code{ion\_mod.F90}:331--340).
\item \CMI{}'s argon exists only as an enumeration.
  \code{ElementNames.hpp}:142--151 declares the ion names, no solver
  references them, and \code{HAS\_ARGON} is defined only under
  \code{ADDITIONAL\_\allowbreak COOLANTS} (\code{:46--49}).
\item \Torus{} omits He II from the opacity, as above, so its
  helium-ionizing continuum is attenuated by neutral helium alone.
\item \Wood{} runs effectively isothermal for its first four iterations.  The
  thermal solve is entered only \code{if(iter.ge.5)} (\code{itbal.f}:27--41),
  and before that \code{ionize2.f}:30 takes the temperature as given, which
  is the \code{Tconst = 8000.} seed of the shipped deck
  (\code{input.params}:6, passed at \code{mcionize.f}:115).
\end{itemize}
...
\subsection{Hydrogen recombination lines}
\label{sec:hlines}

Hydrogen recombination lines deserve their own comparison, for two reasons.
They are the normalization of every published line list, so an error in
H$\beta$ propagates into every ratio quoted against it; and they are the one
emission process every code here computes, which makes them the cleanest
available cross-section through the design choices of the family.  \Cloudy{}
c25.00 is carried in this subsection as well, because it answers the question
in a different category from the other six.  The seven implementations fall
into four method classes.

\emph{Table interpolation in $(\Te,\nelec)$.}  \MoCHII{} reads the Storey \&
Hummer (1995) case-A and case-B tables (11 temperatures $\times$ 13 densities
for case B, 10 $\times$ 9 for case A) and interpolates bilinearly in
$(\log \Te, \log \nelec)$, clamped at the grid edges, for seven lines
(H$\alpha$, H$\beta$, H$\gamma$, H$\delta$, Pa$\alpha$, Pa$\beta$,
Br$\gamma$).  \Mappings{} and \Mthree{} read the same Storey--Hummer
calculation in a richer layout: an absolute H$\beta$ emissivity on 12
temperatures $\times$ 9 densities for \emph{each} case, spline-interpolated in
$\log \Te$, times 90 line ratios (15 lines $\times$ 6 series) on their own
$10 \times 9$ grid (\code{HRECDAT.txt}, \code{HRECRATA.txt},
\code{HRECRATB.txt}; \code{hydro.f}:378--459).  The density grid reaches
$10^{10}\,\mathrm{cm^{-3}}$, against $10^{14}$ in \MoCHII{}'s case-B file and
$10^{10}$ in its case-A file.  \Mocassin{}'s \emph{output} path is also a
table interpolation, of \code{e1bx.d} --- the same Storey--Hummer file
\MoCHII{} parses --- by fourth-order Lagrange in $\log \nelec$ and
$\sqrt{\Te}$, up to $n_{\rm up} = 30$.

\emph{Fixed ratios times a fitted H$\beta$.}  \Mocassin{}'s \emph{iteration}
path reads \code{r1b0100.dat}, whose footer records what it is:
\code{!Te=1.e4 Ne=1.e2 Case B}.  The ratios carry no temperature or density
axis, so H$\alpha$/H$\beta$ is fixed at the tabulated 2.86 whatever the local
state, and the only temperature dependence of any H line is the H$\beta$
normalization.  \Torus{} does the same thing with the same numbers: a ratio
table stated in its own comment to be ``for $T = 10^4$~K and
$n = 10^2\,\mathrm{cm^{-3}}$'' times an H$\beta$ power law
(\code{photoionAMR\_mod.F90}:8022--8043).  The consequence is worth naming,
because it is a physical one: in both codes \emph{the Balmer decrement cannot
respond to temperature at all}.  Against Storey--Hummer at
$\nelec = 100\,\mathrm{cm^{-3}}$ the frozen 2.86 is $-5.9\%$ at 5000~K, right
at $10^4$~K, $+4.1\%$ at $2\times10^4$~K and $+5.9\%$ at $3\times10^4$~K.

\emph{Analytic power-law fits.}  \Wood{} and \CMI{} carry
\emph{numerically identical} coefficients --- \CMI{} is the C++ rewrite, and
this is where that shows most plainly:
\begin{equation}
  \frac{4\pi j(\mathrm{H}\beta)}{\nelec n_{\rm p}}
    = 1.24\times10^{-25}\,T_4^{-0.878},
  \qquad
  \frac{4\pi j(\mathrm{H}\alpha)}{\nelec n_{\rm p}}
    = 2.87 \times 1.24\times10^{-25}\,T_4^{-0.938}
  \label{eq:woodfit}
\end{equation}
in erg\,cm$^3$\,s$^{-1}$ (\code{lines.f}:219--221;
\code{src/EmissivityCalculator.cpp}:224--229), the H$\beta$ coefficient
attributed in both to Storey \& Hummer (1995) and the 2.87 to Osterbrock's
case-B table.  Since the two exponents differ, the ratio drifts as
$T_4^{-0.060}$ rather than staying at 2.87, which is what makes the fit work
as well as it does.

\emph{A solved level population.}  \Cloudy{} has no emissivity table for
hydrogen at all.  Its hydrogen line data are reduced Einstein $A$ coefficients
for $n' \rightarrow n$ transitions (\code{data/hydro\_tpn\_n100.dat}, to
$n = 100$ by default, with an $n = 2000$ set shipped alongside), and the line
emission is whatever the solved H-like iso-sequence level populations give.
The default structure is ten resolved levels (\code{iso.cpp}:192), raised by
the \code{database h-like resolved/collapsed levels} command.  Nothing is
interpolated, so the low-density limit is a result of the solve rather than the
edge of a grid, and the same applies at the top: \Cloudy{} computes through
$\nelec = 10^{13}\,\mathrm{cm^{-3}}$, where a table-reading code has left its
data behind.  The price is that the answer is only as good as the level
structure, which its own test suite states explicitly (below).

The three tabulation-derived families can be compared against one common
standard, because \MoCHII{}'s parse of \code{e1bx.d} is the same
Storey--Hummer case-B table that four of the six table- or fit-based codes
ultimately cite.  Table~\ref{tab:hbetafits}
evaluates each H$\beta$ prescription against it.

\begin{table}[htbp]
\centering
\caption{Each code's H$\beta$ emissivity divided by the Storey \& Hummer
(1995) case-B table value at $\nelec = 100\,\mathrm{cm^{-3}}$, computed from
the coefficients in the sources.  \Wood{} and \CMI{} share
Eq.~(\ref{eq:woodfit}); \Torus{} and \Mocassin{}'s iteration path share
$10^{-0.870\log_{10}\Te + 3.57}$; \Mocassin{}'s output path uses
$2530\,\Te^{-0.833}$ while the accurate fit sits commented out one line above
it (\code{output\_mod.f90}:2060--2063).  All six temperatures listed are
nodes of the table, so the three codes that interpolate it return the table
value there and the interpolation scheme does not enter; what it costs between
nodes is measured in Table~\ref{tab:interp}.}
\label{tab:hbetafits}
\begin{tabular}{lcccc}
\toprule
$\Te$ / K & \MoCHII{}, \Mappings{}, & \Wood{}, \CMI{} &
\Torus{}, \Mocassin{} & \Mocassin{} \\
 & \Mthree{} & & (iteration) & (output) \\
\midrule
...
Two readings follow.  The fits are good: over the range an \HII{} region
actually occupies, both fit families reproduce the table they are fitted to
within a few percent, so the analytic route is a representational choice
rather than an accuracy defect.  And \Mocassin{}'s reported absolute
H$\beta$ is systematically $4.6$--$5.2\%$ below the very table the same
routine interpolates for its ratios, across the whole nebular range.  That
matters for this project specifically: \MoCHII{} takes H$\beta$ from the
table and \Mocassin{} from a fit below it, so a few percentage points of any
$L(\mathrm{H}\beta)$ comparison between the two codes is a coefficient
artifact rather than physics.  Line \emph{ratios} are immune, because
\Mocassin{} divides by its own H$\beta$ before writing them.

\paragraph{What the interpolation scheme costs.}  Reading a table exactly at
its nodes is not the same as agreeing between them, and the three codes that
interpolate the Storey--Hummer grid do it three different ways.  \MoCHII{} is
bilinear in $(\log \Te, \log \nelec)$ on $\log j$, i.e.\ a power law within
each cell (\code{sh95\_mod.f90}:162--175).  \Mappings{} and \Mthree{} take a
natural cubic spline in $\log_{10}\Te$ of $\log_{10} j$ and interpolate
linearly in $\log \nelec$ (\code{hydro.f}:255--265, 435--459).  \Mocassin{}'s
output path uses a five-point Lagrange polynomial in $\sqrt{\Te}$ and
$\log_{10}\nelec$ applied to $\log(j\sqrt{\Te})$, dividing the smoothing
factor out afterwards (\code{output\_mod.f90}:2234--2366,
\code{data maxv/4/ ni/2,3,4,5,6/}) --- the scheme Storey \& Hummer's own
interpolation program uses.

Table~\ref{tab:interp} evaluates the three on the same case-B table.  The
spread is identically zero at the grid nodes and reaches a few tenths of a
percent between them, largest where the grid is coarsest and at the high
densities where the emissivity turns over.  Two observations follow.  The
choice of scheme is not a material source of disagreement between these codes:
at $0.1$--$0.3\%$ it is an order of magnitude below the offset of
\Mocassin{}'s fitted H$\beta$ (Table~\ref{tab:hbetafits}) and below the
$0.4$--$3.6\%$ of the analytic fits.  But the error of the low-order scheme is
\emph{one-sided}, which is the property that matters: the chord lies below the
curve, so \MoCHII{}'s bilinear form is low at every off-node point rather than
scattering about zero, and integrating over a model cannot average it away.

Because \MoCHII{}'s $L(\mathrm{H}\beta)$ sits below the published benchmark
values, that bias is the natural suspect, and it can be measured on the
converged models rather than estimated.  Re-evaluating the same converged state
--- leaf by leaf, at each leaf's own $(\Te,\nelec)$ --- with the two
higher-order schemes in place of the bilinear one changes the
volume-integrated $L(\mathrm{H}\beta)$ by
\begin{equation}
  -0.061\%\ \ (\mathrm{HII40}), \qquad -0.126\%\ \ (\mathrm{HII20}),
  \label{eq:interpbias}
\end{equation}
identically against the spline and against the Lagrange form.  The bias is
indeed systematic and not statistical: of the 136\,168 emitting leaves at
HII40 \emph{every one} is biased in the same direction, with a median of
$-0.065\%$ and a worst case of $-0.217\%$; at HII20, $93.5\%$ are, with a
median of $-0.130\%$.  HII20 carries twice the bias of HII40 because its
temperature falls in the wider $5000$--$7500$~K cell of the table where the
chord error is largest.  The Balmer decrement is less sensitive still,
$\le 0.05\%$, since the bias is common to the two lines and divides out of the
ratio.

Set against a deficit of $3$--$5\%$ (Section~\ref{sec:validation}), the
interpolation algorithm therefore accounts for one part in fifty at HII40 and
one part in thirty at HII20.  The remainder is the photon budget: the
converged models return $0.950$ and $0.970$ of $\QH$ as hydrogen
recombinations, with helium taking $0.112\,\QH$ at HII40, and
$L(\mathrm{H}\beta)$ follows that budget rather than the emissivity table.
Removing the one-sided bias --- a spline or the Storey--Hummer
$\sqrt{\Te}$-smoothed form in place of the chord --- is worth doing because a
systematic error should not be left in a normalization quantity, but it will
not move $L(\mathrm{H}\beta)$ perceptibly.

\begin{table}[htbp]
\centering
\caption{Spread among the three table-interpolation schemes: the largest of
the three $j(\mathrm{H}\beta)$ values divided by the smallest, in percent, on
the Storey \& Hummer (1995) case-B table.  Rows at 5\,000, 10\,000 and
20\,000~K and the column at $\nelec = 10^2\,\mathrm{cm^{-3}}$ are grid nodes,
where all three schemes return the tabulated number and the spread vanishes by
construction.  7024 and 8211~K are the converged benchmark temperatures.}
\label{tab:interp}
\begin{tabular}{lcccc}
\toprule
& \multicolumn{4}{c}{$\nelec$ / cm$^{-3}$} \\
\cmidrule(l){2-5}
$\Te$ / K & $10^{2}$ & $3\times10^{3}$ & $10^{5}$ & $3\times10^{6}$ \\
\midrule
...
\paragraph{What a solved atom says about the tables.}  Having one code in the
comparison that does not read the tables makes it possible to ask how good the
tables are, which is otherwise unanswerable from inside this family.  Two
measurements were made with \Cloudy{} c25.00 for this memo, both single-zone,
constant-temperature, hydrogen-only models with 30 resolved and 70 collapsed
levels, run in the configuration of its own case-B reference test
(\code{case b hummer no photoionization no pdest}, \code{no scattering escape},
\code{no induced processes}).

First, at the table's own grid point, $\Te = 10^4$~K and
$\nelec = 10^2\,\mathrm{cm^{-3}}$, the solved populations sit slightly
\emph{below} the tabulated case-B values, and by an amount that grows along the
series:
\begin{center}
\begin{tabular}{lccc}
\toprule
& H$\beta$ & H$\alpha$/H$\beta$ & Pa$\alpha$/H$\beta$ \\
\midrule
Storey \& Hummer (1995) & $1.2350\times10^{-25}$ & 2.8640 & 0.3387 \\
\Cloudy{} c25.00        & $1.2321\times10^{-25}$ & 2.8452 & 0.3319 \\
difference              & $-0.23\%$ & $-0.65\%$ & $-2.00\%$ \\
\bottomrule
\end{tabular}
\end{center}
This is the quantitative version of the statement its test deck makes in words:
the deviation is attributed there to the principal quantum number
specification, 30/70 in the run against 100/1 in the table.  The practical
reading for a table-based code is reassuring in the abstract and cautionary in
the detail --- the normalization line agrees to a quarter of a percent, while a
ratio between widely separated series does not, at the level a Balmer/Paschen
decrement measurement would notice.

Second, and this is what settles the question the table cannot answer, the same
model run down to $\nelec = 10^0\,\mathrm{cm^{-3}}$ shows the hydrogen
coefficients essentially flat, which is the measurement quoted in the
density paragraph above.  Its own validation suite tests
$\log \nelec = 2, 4, 5, 8, 10$ and $13$; the lowest is the Storey--Hummer floor,
because below it there is nothing tabulated to validate against.  A code that
solves the populations does not need the floor, and a code that reads the table
needs a defensible rule at it.

\paragraph{Case A and case B.}  This is where the six differ most, and where
the ordering is not the one the codes' ages would suggest.  \Mappings{} has
the most physical treatment of any code here: the case parameter is not a
switch but a \emph{continuous} quantity computed zone by zone from the
Lyman-$\gamma$ escape probability,
$\mathrm{casab} = 1 - e^{-0.8597\,x}$ with $x$ the line-transfer path
multiplier (\code{functions.f}:21--71), and the code interpolates
logarithmically between the case-A and case-B tables at that value.  The same
escape quantity that sets the parameter also attenuates the lines in the
diffuse field, so the treatment is self-consistent.  \Mthree{} inherits the
routine and the data but not the logic: its three-dimensional driver
hardcodes \code{caseab(1)=1.0d0} with the escape-probability call commented
out one line below (\code{calpdf.f}:45--46, 54--55), so the connected path is
pure case B everywhere and the Lyman series above Ly$\alpha$ is identically
zero even in optically thin gas.  \MoCHII{} sits between the two: two
explicit parameters, \code{par\%case\_ab} for the ionizing continuum and
\code{par\%line\_case} for the line tables, both defaulting to B, with the
second added precisely because tying them together silently loaded case-A
line tables whenever the diffuse field was switched on.

The other three each conflate the two questions in a different way.  \Wood{}
has it right by convention and says nothing about it: its lines are case B
(hardcoded, no switch) while its continuum uses the case-A total
recombination coefficient with an explicitly transported diffuse field --- the
same pairing \MoCHII{} arrived at, with no parameter and no comment marking
the distinction.  \CMI{} hardwires case-B line coefficients while the
continuum case follows from two uncoordinated defaults, a diffuse handler
that is off and a recombination coefficient that is the case-A total.
\Cloudy{} is in a category of its own here, and it is the cleanest of the
seven: the case is neither a table nor a flag on the data but a \emph{transfer
condition}.  \code{case b} sets the minimum Lyman-line optical depth to
$10^{5}$ and case A or C to $10^{-5}$ (\code{parse\_caseb.cpp}:19--41), and
\code{rt\_tau\_reset.cpp}:137--144 then clobbers the Ly$\alpha$ optical depth
to that value while every other line is transferred normally.  The command also
carries three options whose only purpose is to \emph{emulate} the tabulated
assumptions --- \code{hummer} (``no collisions from n=1, 2''),
\code{no photoionization} and \code{no pdest} --- and its own case-B
reference test uses all three, plus \code{no scattering escape} and
\code{no induced processes}, with 30 resolved and 70 collapsed levels.  The
test's closing comment is the honest summary of what a case-B table is:
``This is close to the best and most complete model of H I that the code can
do.  The predicted results still deviate from the tabulated `Ca B' results
because of the principal quantum number specifications (30/70 here; 100/1 in
table).''  In other words \Cloudy{} has to switch physics off to approach the
tables, and the same test finds departure coefficients of $0.46$--$1.79$ for
levels $n = 30$ to $530$ at $\nelec = 10^{2}\,\mathrm{cm^{-3}}$, which is why
the solved populations are the more general statement.

\Torus{} exposes a \code{caseB} flag that changes neither the lines nor the
transport but replaces the hydrogen recombination coefficient with a
temperature-independent constant $2.7\times10^{-13}\,\mathrm{cm^3\,s^{-1}}$,
and warns rather than refuses when it is combined with a transported diffuse
field.  \Mocassin{} offers case B only.

\paragraph{Density dependence, and what happens below the table.}
\MoCHII{}, \Mappings{}, \Mthree{} and \Mocassin{}'s output ratios carry it;
\Wood{}, \CMI{}, \Torus{} and \Mocassin{}'s iteration path do not;
\Cloudy{} carries it inherently, since the populations are solved rather than
read.  That difference has a consequence the table-based codes have to answer
for: the Storey--Hummer grids begin at
$\nelec = 10^{2}\,\mathrm{cm^{-3}}$ (the original CDS files as well as the
repackaged ones), so every code that reads them needs a rule below that
density, and \MoCHII{}'s rule is to clamp
(\code{sh95\_mod.f90}:163).  Two measurements bound what the clamp costs.
From the bottom slope of the table itself, extrapolating the collisional term
linearly in $\nelec$, the $\nelec \rightarrow 0$ error is $\le 0.08\%$ for
every H\,\textsc{i} and He\,\textsc{ii} line in it.  Independently,
\Cloudy{} --- which has no floor --- gives $+0.065\%$ for H$\beta$,
$+0.108\%$ for H$\alpha$ and $+0.03$ to $+0.05\%$ for the members up to
$n = 17 \rightarrow 2$ between $\nelec = 10^{2}$ and $10^{0}\,
\mathrm{cm^{-3}}$ at $10^{4}$~K.  The two agree that for hydrogen the
low-density limit is already reached at the table's first column, and the cost
of the clamp on an integrated luminosity is smaller still: replacing it with
the $\nelec \rightarrow 0$ limit moves $L(\mathrm{H}\beta)$ by $-0.0002\%$
at HII40, $-0.0003\%$ at HII20 and $-0.031\%$ in the low-density
circumgalactic demonstration where every emitting cell is below the floor.

\begingroup\small\setlength{\tabcolsep}{4pt}
\begin{longtable}{P{0.115\textwidth}P{0.20\textwidth}P{0.18\textwidth}P{0.10\textwidth}P{0.10\textwidth}P{0.14\textwidth}}
\caption{Hydrogen recombination lines, one row per code.  ``Case mechanism''
is how the code decides between case A and case B, which is the axis on which
no two of them agree.}
\label{tab:hlines}\\
\toprule
Code & Emissivity source & Case mechanism & $\nelec$ depend. & H collis.\ term & Transported \\
\midrule
\endfirsthead
\multicolumn{6}{l}{\small\itshape Table~\ref{tab:hlines} continued.}\\
\toprule
Code & Emissivity source & Case mechanism & $\nelec$ depend. & H collis.\ term & Transported \\
\midrule
\endhead
\bottomrule
\endfoot
\MoCHII{} & SH95 case-A/B tables, bilinear in $(\log\Te,\log\nelec)$ &
\code{line\_case}, separate from the continuum \code{case\_ab} & yes &
optional, 25-level atom & no \\

\Mappings{} & SH95 absolute H$\beta$ + 90 ratios, spline in $\log\Te$ &
continuous, from the Ly$\gamma$ escape probability & yes & yes, with
case-dependent cascade & yes, outward-only with escape factors \\

\Mthree{} & the same routine and data & hardcoded case B in the 3D driver &
yes & yes, as above & 5--200 eV band only, so Lyman lines only \\

\Mocassin{} & ratios frozen at $(10^4$~K, $10^2)$ in the iteration; SH95
interpolated at output & case B only, no switch & output ratios only & no &
energy removed, photon not propagated \\

\Wood{} & analytic fits to SH95, Eq.~(\ref{eq:woodfit}) & case B hardcoded,
continuum case A & no & no & no \\

\CMI{} & the same coefficients & case B hardwired; continuum case from two
uncoordinated defaults & no & no & no \\

\Torus{} & ratios frozen at $(10^4$~K, $10^2)$ times a fitted H$\beta$ &
\code{caseB} changes only the recombination coefficient & no & no & yes, into
the diffuse redraw spectrum \\

\Cloudy{} & none: solved H-like level populations, $n \le 100$ by default &
a Lyman-line optical depth, $10^{5}$ against $10^{-5}$ & inherent & inherent &
1D line transfer \\
\end{longtable}
\endgroup

\emph{Helium is the exception, and it is not a small one.}  The
Porter et al.\ (2012/2013) He\,\textsc{i} table starts one decade lower,
$\nelec = 10^{1}\,\mathrm{cm^{-3}}$, and its 10833~\AA{} line does not
flatten there: across the lowest decade of its own grid the coefficient moves
$+19.1\%$, against $\le 0.8\%$ for the five singlet lines in the same table.
\Cloudy{} confirms the effect independently, $+10.5\%$ from
$\nelec = 10^{1}$ to $10^{2}\,\mathrm{cm^{-3}}$ and a further $+1.1\%$ from
$10^{0}$ to $10^{1}$, against $\le 0.04\%$ for 5876~\AA{}.  The reason is
structural rather than numerical: the lower level of 10830 is the metastable
$2^3$S, whose population is set collisionally, so this line has no
density-independent limit in the regime the table covers --- which is precisely
why the $2^3$S population is computed explicitly elsewhere in \MoCHII{} rather
than inferred from a case-B emissivity.  Clamping it is therefore a real error
in diffuse gas, at the ten-percent level for $\nelec < 10\,\mathrm{cm^{-3}}$,
and the honest statement is that the tabulated 10833 emissivity should not be
used below the Porter floor at all.  The cost is small in an \HII{} region
and not small in a planetary nebula: from the same table, $j(\mathrm{H}\beta)$
rises $0.4\%$ from $\nelec = 10^2$ to $10^4\,\mathrm{cm^{-3}}$ at $10^4$~K
and $1.6\%$ to $10^6$, while H$\alpha$/H$\beta$ falls $2.0\%$ by $10^6$ and
$5.7\%$ by $10^8$.  This is the same axis as the metal-line one of
Section~\ref{sec:densitycool}, and two codes are internally inconsistent
across it: \CMI{} and \Wood{} both solve their metal lines from level
populations at the local $\nelec$ while leaving the hydrogen lines
density-independent.

\paragraph{Collisional excitation.}  \Mappings{} and \Mthree{} are the only
codes that add a collisional term inside the line calculation itself, and
they do it carefully: the excitation rate to each level (Anderson et al.\
2000 collision strengths for $n \le 5$, Giovanardi et al.\ fits for
$6 \le n \le 16$) times case-dependent cascade branching fractions read from
their own data file, added to the recombination brightness
(\code{hydro.f}:756--812), with the same energy booked as cooling and not
double counted.  \MoCHII{} has the equivalent as an option
(\code{par\%h\_coll\_effects}: a 25-level H\,\textsc{i} atom solved
collisionally, its Balmer output written as separate rows so the
recombination row stays a pure recombination quantity).  \Cloudy{} needs no such term as an addition: collisional excitation is one
of the rates in the level solve, which is what makes its Balmer emission
depart from case B at all.  \Wood{}, \CMI{},
\Mocassin{} and \Torus{} add nothing --- consistent with the fact, recorded in
the cooling audit, that none of them carries H\,\textsc{i}
collisional-excitation cooling either.

\paragraph{Transport.}  The lines are diagnostics in \MoCHII{}, \Wood{} and
\CMI{}: nothing re-emits a Balmer photon, and in \CMI{} the in-run
emissivity call is commented out altogether, leaving a separate
post-processing mode as the only route.  \Mocassin{} occupies a middle
position that is easy to misread: the recombination-line energy is summed
into the diffuse budget so that it correctly leaves the ionizing field, but a
packet that lands in the line channel is given $\nu = 0$ and returns without
being propagated --- the energy is removed, the photon is not transported.
\Torus{} genuinely transports them, writing the lines into the diffuse redraw
spectrum so an absorbed ionizing packet can re-emerge as an H$\alpha$ photon.
\Mappings{} transports all of them into its outward-only diffuse field with
escape and dust-retention factors.  \Mthree{} transports the subset that
falls in its hardcoded 5--200~eV Monte Carlo band, which admits the Lyman
series and excludes every Balmer and redder line, and it applies no escape
factor to them.

\paragraph{What each code refuses to compute.}  \Wood{} and \CMI{} return
zero emissivity unless $x_{\rm HI} < 0.2$ and $\Te > 3000$~K, so their
emissivity maps are blank across the ionization front and through the neutral
gas by construction; \CMI{}'s Python twin uses $0.25$ where the compiled code
uses $0.2$.  \MoCHII{} warns once when a cell exceeds the 30\,000~K table
edge and clamps rather than extrapolating.  \Mappings{} zeroes all hydrogen
lines outside $10 < \Te < 10^6$~K and extrapolates its ratio tables above
$3\times10^4$~K, where they end.

\paragraph{Helium, for contrast.}  The same audit on helium separates the
codes further.  \MoCHII{} carries Storey--Hummer He\,\textsc{ii} tables in
both cases and Porter et al.\ (2012/2013) He\,\textsc{i} emissivities;
\Mappings{} and \Mthree{} carry the same He\,\textsc{ii} calculation and
Porter et al.\ (2007), an earlier release from the same group;
\Mocassin{} uses a case-A He\,\textsc{ii} table for the transported channel,
which is the correct use of a case-A table for photons that ionize hydrogen.
\Torus{} loads three helium tables at startup --- Storey--Hummer
He\,\textsc{ii}, Benjamin et al.\ (1999) He\,\textsc{i}, and a
Storey--Hummer H\,\textsc{i} table it never uses --- and the routine that
would consume them is commented out in its entirety along with both of its
call sites, so \Torus{} emits no helium recombination line of any kind.

\paragraph{Two small findings, recorded for completeness.}  \Mocassin{} adds
its Lyman-$\alpha$ luminosity onto an existing slot of the H\,\textsc{i}
array rather than a spare one, in both of its paths, and in the output path
that slot reaches the user: the shipped HII40 result reports a row at
$24.93\,L(\mathrm{H}\beta)$ carrying the $30\rightarrow8$ wavelength of
62\,802~\AA{}, which is Lyman $\alpha$ under a mid-infrared label.  And in
both \Mappings{} and \Mthree{} the He\,\textsc{ii} collisional add-on is
evaluated with hydrogen's nuclear charge (\code{atom=2} followed by
\code{nz=mapz(1)}, \code{hydro.f}:789--791), so its transition energy carries
$Z^2 = 1$ instead of $4$.  The practical effect is negligible, since He$^+$
ground-state excitation is Boltzmann-suppressed at nebular temperatures, but
it would matter in coronal gas.
