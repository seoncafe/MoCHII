...
$1.01\times$ \Cloudy{}'s, and raises the converged benchmarks to 8210 and
7024~K, matching \Cloudy{} c25.00 (8210, 6910~K); the zero-density fits are
retained as the \code{low\_density} option.  A comparison of the cooling
...
These are the zero-density-limit (\code{low\_density}) temperatures; the
density-suppressed default of Section~\ref{sec:limit} raises them further, to
8210 and 7024~K, to match \Cloudy{} c25.00.
...
tables.  The ``after'' column is the zero-density (\code{low\_density})
cooling; the density-suppressed default raises $\langle T\rangle$ to 8210 and
7024~K (Section~\ref{sec:limit}).}
\label{tab:bench}
\begin{tabular}{lcccc}
\toprule
...
Run with the fits used directly, the consequence for the benchmarks was a
slight cooling bias at $\nH = 100$: both models sat at the cool edge of the
published range, and HII20 fell $0.4\%$ below its floor.  \MoCHII{} removes it
by suppressing the line cooling to the local density.  A $(T, \nel)$ table,
...
and the converged benchmarks rise to \textbf{8210}~K at HII40 --- matching
\Cloudy{} c25.00's 8210~K to $0.4$~K --- and 7024~K at HII20 ($+1.6\%$ against
...
Table~\ref{tab:bench}, essentially at \MoCHII{}'s density-suppressed default
$8210$~K and \Cloudy{} c25.00's $8210$~K (and above \MoCHII{}'s zero-density
...
and raises the converged benchmarks to 8210 and 7024~K, matching \Cloudy{}
c25.00 (8210, 6910~K); the zero-density fits are kept as the \code{low\_density}
option.
