...
time.  The remaining known limitation is that $\Lambda(T)$ carries no density
suppression.  A comparison of the cooling inventories of the other Monte Carlo
photoionization codes is included; it turned up defects on their side as well,
and one omission common to all five codes.
\end{abstract}
...
of Ercolano et al.\ (2003, their Tables~4 and 5).  \MoCHII{} matched the published
...
\begin{table}[htbp]
\centering
\caption{The Lexington/Meudon benchmarks before and after the cooling fix.
``published'' is the range over the nine codes of the published compilation; C25 is \Cloudy{} c25.00 run on its own unmodified test-suite deck.
\MoCHII{} runs use threshold-aligned 32 ionizing bins, the explicit Monte
Carlo diffuse field with the \HeI{} channels, and case-B recombination-line
tables.}
\label{tab:bench}
\begin{tabular}{lcccc}
\toprule
 & before & after & C25 & published \\
\midrule
\multicolumn{5}{l}{\textbf{HII40}} \\
$L({\rm H}\beta)/10^{37}$ erg\,s$^{-1}$ & --- & 1.95 & 2.06 & 2.01--2.10 \\
$\langle T[N_{\rm p}N_{\rm e}]\rangle$ / K & 9159 & \textbf{7894} & 8210 & 7720--8199 \\
$T_{\rm inner}$ / K                     & ---  & 7270 & 7812 & 7370--8318 \\
$R_{\rm out}/10^{19}$ cm                & not predicted & 1.43 & 1.47 & 1.45--1.47 \\
$\langle {\rm He^+}\rangle/\langle {\rm H^+}\rangle$ & 0.667 & 0.715 & 0.741 & 0.715--0.829 \\
recombinations / $Q_{\rm H}$            & 0.888 & 0.953 & --- & --- \\
\midrule
\multicolumn{5}{l}{\textbf{HII20}} \\
$L({\rm H}\beta)/10^{36}$ erg\,s$^{-1}$ & --- & 4.70 & 4.99 & 4.83--5.04 \\
$\langle T[N_{\rm p}N_{\rm e}]\rangle$ / K & 7370 & \textbf{6376} & 6910 & 6402--6749 \\
$T_{\rm inner}$ / K                     & --- & 6752 & 7155 & 6562--7224 \\
$R_{\rm out}/10^{18}$ cm                & --- & 8.69 & 8.97 & 8.70--9.01 \\
$\langle {\rm He^+}\rangle/\langle {\rm H^+}\rangle$ & 0.047 & 0.047 & 0.043 & 0.034--0.077 \\
...
$[\OIII{}]$ 5007+4959 was $4.28$ before the fix.}
\label{tab:lines}
\begin{tabular}{lccc}
\toprule
...
%==========================================================================
\section{Remaining limitation: $\Lambda(T)$ carries no density suppression}
\label{sec:limit}
...
The consequence for the benchmarks is a slight cooling bias at $\nH = 100$,
and it is visible: both models now sit at the cool edge of the published
range, and HII20 sits $0.4\%$ below its floor.  The fix is a Tier-1 table in
$(T, \nel)$ rather than $T$ alone, which is already on the plan for the
planetary-nebula densities where the effect is much larger.

\paragraph{Consequence for the PDR path (needs re-baselining).}  The
photodissociation-region test \code{tests/pdr/pdr\_L5.in} shows the expected
signature.  The PDR shell itself is unchanged --- $\nel = A_{\rm C}\nH$ plus
$6\%$ from S, Mg and Fe, $\Te = 258$~K, $\CII{} = 1.000$ --- because there
$\nel \sim 2\times10^{-2}$~cm$^{-3}$ is far below $\ncrit$ and the low-density
limit is the correct limit.  The \HII{} interior, however, is now about $17\%$
cooler, because that is where $\CII{}$ dominates the carbon budget at
$\nel \sim 110$ and the $[\CII{}]$ 158~$\mu$m overestimate bites.  The stored
PDR gate references and the figures and examples built on them need
refreshing, and \code{tests/g2a\_thermal/check\_g2a.py} still refers to a
coefficient file under its former name.
...
far-infrared fine-structure lines saturate above their critical density.  This
is precisely what \MoCHII{}'s thermal balance does not do
(Section~\ref{sec:limit}): on the fixed \Cloudy{} state \MoCHII{}'s
$\nel \to 0$ fits deliver $1.116\times$ \Cloudy{}'s line cooling where its own
$n$-level solve at the local $\nel$ delivers $1.011\times$, a difference that
is a factor $4.5$ on $[\CII{}]$ 158~$\mu$m alone.  \Mocassin{} has that
suppression in the thermal balance; \MoCHII{} has it only in the line output.
...
\paragraph{Net comparison with \MoCHII{}.}  Two differences dominate, and
within each code they work against each other.  \Mocassin{} solves the level
populations at the local $\nel$ inside the thermal balance, which \MoCHII{}'s
$\Lambda(T)$ cannot do and which removes about $10\%$ of the line cooling at
$\nH = 100$~cm$^{-3}$ (Section~\ref{sec:limit}); but its collision strengths
are $15$--$22\%$ higher than the current ones on three of the four largest
metal coolants, which adds cooling.  \MoCHII{} has the current collision
strengths and the missing suppression, the same two effects with the signs
exchanged.  Neither code's temperature can therefore be attributed to a single
data difference.  On channels rather than data, \MoCHII{} carries three that
\Mocassin{} does not --- collisional-ionization cooling, metal free-free with
the net ion charge, and H-impact fine-structure cooling for the
photodissociation-region path (\Mocassin{}'s $\Upsilon$ tables are
electron-impact only) --- and \Mocassin{} carries two that \MoCHII{} does not,
the gas--grain collisional term and the density-dependent level populations.
...
unlike \MoCHII{}'s $\Lambda(T)$, carries collisional de-excitation inside the
thermal balance and saturates the far-infrared fine-structure lines above their
critical density --- the limitation of Section~\ref{sec:limit}.  Two
...
Table~\ref{tab:bench}, above \MoCHII{}'s post-fix $7894$~K and just under
\Cloudy{} c25.00's $8210$~K.  \code{benchmarks/HII\_region/} also holds a
...
balance that \MoCHII{} lacks, on transition energies taken from observed levels
as \MoCHII{}'s rebuilt fits now are, and from the same routine that makes its
line output; it has the gas--grain collisional term as an option; and it puts
...
H/He channels except for the last three rows: the density-dependent level
populations, which it has only in its Tier-2 line output; the gas--grain term,
which \Mocassin{} carries and \Torus{} carries as an option; and \HeI{}
collisional excitation, which no code carries.}
\label{tab:codes}
\footnotesize
...
level populations at the local $\nel$ & yes & yes & yes & yes & Tier-2 only \\
...
budget, the outer radius and the line ratios along with it.  The remaining
known limitation is the absence of density suppression in $\Lambda(T)$, which
now biases the benchmarks slightly cool and calls for a $(T, \nel)$ table.
...
channels, with three exceptions.  \HeI{} collisional-excitation line cooling is
absent from Wood's code, from \CMI{}, from \Mocassin{}, from \Torus{} and from
\MoCHII{}, and is worth $0.062\%$ of the HII40 budget.  \Mocassin{} carries a
gas--grain collisional term and \Torus{} carries one as an option, of order
$10^{-4}$ of the line cooling at
$\nH = 100$~cm$^{-3}$.  And all four other codes solve their metal level
populations at the local $\nel$ inside the thermal balance, which \MoCHII{}
does only in its Tier-2 line output --- the limitation of
Section~\ref{sec:limit}.  It also turned up defects on their side --- Wood's \NI{} and
