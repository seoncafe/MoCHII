...
'le' run with NO src_lum DERIVES src_lum ~ 1e37 and reproduces the shape run
(two_temp_file) per bin.  The SAME physical spectrum is written in every unit
system, so the 'le'/'lnu'/'llam_a'/'llam_um' runs must agree:

  le_2src.txt      cols  E [eV]      L_E1 L_E2   [erg/s/eV]   (E grid, ascending)
  llama_2src.txt   cols  lam [A]     L_l1 L_l2   [erg/s/A]    (lambda grid, asc.)
  llamum_2src.txt  cols  lam [um]    L_l1 L_l2   [erg/s/um]   (lambda grid, asc.)
...
slightly from the E-grid 'le' run - the residual the G-llam gate bounds (<0.5%).
"""
import os
import numpy as np
...
def main():
    ...
    # --- 'le': E grid (same 400-pt log grid as two_temp_spec.txt) ---
    E = np.logspace(np.log10(10.0), np.log10(110.0), 400)
    Le1 = planck_nu(E, T1) * K1        # L_E [erg/s/eV], ionizing int = 1e37
    Le2 = planck_nu(E, T2) * K2
    write_cols(os.path.join(HERE, "le_2src.txt"), E, [Le1, Le2],
               "spectrum_type=le  col1 E [eV]  col2 L_E(3e4)  col3 L_E(5e4) "
               "[erg/s/eV]; ionizing int = 1e37 each")

    # --- 'llam_a' / 'llam_um': uniform-in-lambda grid over the same E range ---
    ...
    write_cols(os.path.join(HERE, "llama_2src.txt"), lam_a, [Lla1, Lla2],
               "spectrum_type=llam_a  col1 lambda [A]  col2 L_lam(3e4)  "
    ...
    write_cols(os.path.join(HERE, "llamum_2src.txt"), lam_um, [Llu1, Llu2],
               "spectrum_type=llam_um  col1 lambda [um]  col2 L_lam(3e4)  "
               "col3 L_lam(5e4) [erg/s/um]")

    # --- external 'le' J_E file: band-integrated int J_E dE = 1e-5 ---
    Kj = J_TARGET / ion_integral(4.0e4)
    Je = planck_nu(E, 4.0e4) * Kj
    write_cols(os.path.join(HERE, "extje.txt"), E, [Je],
               "spectrum_type=le (external)  col1 E [eV]  col2 J_E "
               "[erg/s/cm^2/sr/eV]; ionizing int = 1e-5")

    print(f"K1={K1:.6e} K2={K2:.6e} Kj={Kj:.6e}")
    print("wrote le_2src.txt, llama_2src.txt, llamum_2src.txt, extje.txt")
