...
| `grain_kext` | the extinction SEDust serves from `data/kext_*.dat` against the size integral over the same grain model, on the grids MoCHII builds (astrodust EUV 100/150 eV + unextended, DL07 EUV, Zubko): table value at the T-matrix nodes, interpolation error below 0.0912 um, band-averaged `s_abs` and `C_ext(lambda_ref)` (`check_kext_table.f90`, `run_check.sh`) |
