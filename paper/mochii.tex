...
These switches default off so the recorded gates reproduce, but the benchmark
runs of Section~\ref{sec:hii} enable all of them, so every metal feedback
\MoCHII{} implements is active in those numbers; the trace separation
describes the default configuration and the solution method, not the physics
of the benchmark.
...
The same near-cancellation makes the cell iteration marginally stable in
partially neutral gas, which is recorded with its measured pass counts and
must be resolved before a photodissociation-region result is quoted.  The one
