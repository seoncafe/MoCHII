...
current \textsc{Cloudy} c25.00 run of the same models to $0.2\%$ (HII40)
and $0.1\%$ (HII20), with the He$^{+}$/H$^{+}$ ratio and the outer radius
...
\paragraph{Grouped compatibility mode.}  The historical grouped mode
...
$\langle\mathrm{He^{+}}\rangle/\langle\mathrm{H^{+}}\rangle$ is 0.703 at
...
\emph{Temperature.}  The weighted temperature is 8194~K at HII40 and
6916~K at HII20.  Both match a current \textsc{Cloudy} c25.00 run of the
same two models --- 8210~K and 6910~K --- to $0.2\%$ and $0.1\%$, and both
...
published spread at both, 7538~K at HII40 against a published 7370--8318~K
...
$\nelec$, Section~\ref{sec:cooldata} --- gives the 8194 and 6916~K that
...
% explicit diffuse field, Q(H) matched to the specification, 2^25 packets,
% conv_crit='vol'); line entries are the L/L(Hbeta) column of
...
default reaches 8194~K, matching \textsc{Cloudy} to $0.2\%$
...
$4.71\times10^{36}\,\mathrm{erg\,s^{-1}}$ are about $5\%$ below the
...
ancestor GF.  The continuous-energy results retain the broad pattern of the
published comparison.  The low-ionization edge line [O\,\textsc{i}]
6300+6363 is 0.0169 at HII40, above the published 0.0059--0.012 range; it is
particularly sensitive to the thin partly neutral edge.  The full, updated
comparison is tabulated rather than reduced to a count of entries inside a
historical spread.
...
[O\,\textsc{iii}] 5007+4959 is $0.96\times$, its auroral 4363 $0.96\times$
...
\MoCHII{} 0.322, so what would be a $2.4\times$ excess against the 1990s
column is a $6\%$ difference against the current one.  Sulfur at HII20 is
the same story in the other direction: [S\,\textsc{iii}] 18.7\,$\mu$m
(0.215 against a published 0.34--0.46) and [S\,\textsc{iii}] 9532+9069
(0.303 against 0.465--0.604) fall \emph{below} every published code, and so
...
\textsc{Cloudy} c25.00 to $0.2\%$ at HII40 and $0.1\%$ at HII20 --- both
...
density, the
$n_{\mathrm{p}}n_{\mathrm{e}}$-weighted temperature is 8194~K at HII40 and
6916~K at HII20, matching a current \textsc{Cloudy} c25.00 run of the same
two models (8210 and 6910~K) to $0.2\%$ and $0.1\%$; both models are
