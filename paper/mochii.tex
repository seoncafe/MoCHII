...
(ionized volume $+0.60\%$ versus an exact 1D reference; AMR identical to
a uniform grid), a thermal H/He nebula (median $0.97\%$ in $\Te$), and
...
4\pi)^{1/3} = 3.0365$~pc; a uniform level-6 octree gives 3.0546~pc
($+0.60\%$), and a front-refined octree with $3.5\times$ fewer cells
reproduces it to four decimals, while a center-refined control (coarsest
cells at the front) deviates by $+2.34\%$ --- the ionization front is the
one place needing resolution (Figure~\ref{fig:g1}).  With the thermal
balance on, $\Te(r)$ matches the 1D thermal reference to a median $0.97\%$
and a mean bias $+1.02\%$ (Figure~\ref{fig:g2a}).  Both figures are set by the
reference rather than by the transport.  The 1D reference carries the
\citet{huignedin1997} case-B recombination coefficient while the run uses the
Badnell total with the Milne ground-level rate, and the two differ by $1.2\%$ at
the 19--24~kK of this test; running the same case with the reference's own
coefficients gives a median $0.78\%$, so $0.19$ percentage points is that
mismatch.  The remainder is the reference's own free-free Gaunt approximation,
which it documents as $1$--$2\%$ of a cooling term worth $\sim$$10\%$ of the
total.  The comparison is therefore a bound on the 1D reference, not a
resolution of the Monte Carlo solution.  Without metals the
...
$4.5\times$ fewer cells, and both improve on level 6 ($+0.60\%$), as
...
MW dust gives $R_{\mathrm{eff}} = 2.205$~pc versus the reference 2.198~pc
($+0.31\%$), reproducing the classic dusty reduction $R/R_0 = 0.722$
...
sphere to $+0.60\%$, and a thermal H/He nebula to $0.97\%$.  On the
