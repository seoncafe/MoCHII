...
and a mean bias $+1.02\%$ (Figure~\ref{fig:g2a}).  Part of that offset is a
data-source difference rather than a transport error: the 1D reference carries
the \citet{huignedin1997} case-B recombination coefficient while the run uses the
Badnell total with the Milne ground-level rate, and the two differ by $1.2\%$
at the 19--24~kK of this test.  Running the same case with the reference's own
coefficient set gives a median $0.78\%$, so $0.19$ percentage points of the
deviation is that mismatch; the remainder is not, and is an open item.  Without
metals the
