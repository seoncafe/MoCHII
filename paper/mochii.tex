...
current \textsc{Cloudy} c25.00 run of the same models to $0.4$~K (HII40)
and $1.6\%$ (HII20), with the He$^{+}$/H$^{+}$ ratio and the outer radius
...
$N = 2^{17}$, $66.5\times$ at $2^{18}$, $141\times$ at $2^{19}$, and
$222.5\times$ at $2^{20}$.  The two launch sets cost the same wall time, and
the quasi-random result carries no significant bias.
...
observation.  All three panels share one logarithmic surface-brightness
scale, so the extended scattered halo is directly comparable to the compact
direct source.}
\label{fig:peel}
\end{figure*}
...
coefficient returns to the fit exactly.  This density-suppressed cooling
...
10830~\AA\ triplet, whose absorption traces the extended atmospheres of
close-in exoplanets and the neutral gas of nebulae.  \MoCHII{} solves its
...
resulting density the line-center optical depth of the 10830~\AA\ doublet is
formed.  The diagnostic is pure post-processing, with no feedback on the
ionization or thermal balance.

Figure~\ref{fig:hei10830} shows the two observables for a dense compact
example ($\nH = 10^4\,\mathrm{cm^{-3}}$, $R = 0.15$~pc, $\Te$ fixed at
$10^4$~K): the $2^3$S column density, which peaks at
$N = 3.68\times10^{15}\,\mathrm{cm^{-2}}$, and the line-center optical depth
$\tau_0$ of the blended $J' = 1, 2$ doublet, with the $\tau_0 = 1$ contour
marked.  In the core $\tau_0$ reaches $4.4\times10^{3}$, so the region is
optically thick at 10830~\AA\ and the optically-thin diagnostic
underestimates the true absorption there; for the full line profile the
converged $2^3$S density is instead handed to a resonant-scattering
Lyman-$\alpha$/10830 transfer code.

\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/hei10830.pdf}
% source: paper/python/make_hei10830.py, from examples/hei_metastable/hei_high_density
\caption{The \HeI{} $2^3$S metastable diagnostic for a dense compact
example ($\nH = 10^4\,\mathrm{cm^{-3}}$, $R = 0.15$~pc, $\Te = 10^4$~K),
as line-of-sight projection maps.  Left: the $2^3$S column density.  Right:
the 10830~\AA\ line-center optical depth $\tau_0$ of the blended $J' = 1, 2$
doublet, with the $\tau_0 = 1$ contour (cyan); the core is optically thick,
where the optically-thin diagnostic breaks down.}
\label{fig:hei10830}
\end{figure*}
...
$\langle\mathrm{He^{+}}\rangle/\langle\mathrm{H^{+}}\rangle$ is 0.702 at
HII40, just below the published 0.715--0.829 (median 0.767) and the current
\textsc{Cloudy} value (C25 0.741, itself under the published median), and
0.046 at HII20, near the published median of 0.048 inside a spread of
0.034--0.077.

\emph{Temperature.}  The weighted temperature is 8210~K at HII40 and
7024~K at HII20.  Both match a current \textsc{Cloudy} c25.00 run of the
same two models --- 8210~K and 6910~K --- to $0.4$~K and $1.6\%$, and both
...
published spread at both, 7562~K at HII40 against a published 7370--8318~K
and 7048~K at HII20 against a published 6562--7224~K.  The converged
MOCASSIN 2.02 run of the HII40 model gives 8176~K, inside the published
spread and $6\%$ above MOCASSIN's own published entry.  The two contemporary
codes therefore agree closely and both sit above the quarter-century-old
published temperatures, whose columns carry the atomic data of their time.
...
$\nelec$, Section~\ref{sec:cooldata} --- gives the 8210 and 7024~K that
...
default reaches 8210~K, matching \textsc{Cloudy} to $0.4$~K
...
$L(\mathrm{H}\beta) = 1.94\times10^{37}$ and
$4.68\times10^{36}\,\mathrm{erg\,s^{-1}}$ are about $5\%$ below the
published medians (some $4\%$ below the lowest published values),
consistent with the $4.7\%$ and $2.9\%$ of $\QH$ that the recombination
...
with all published codes, a current \textsc{Cloudy}~c25.00 column, and the
...
ancestor GF.  Of the twenty lines tabulated at HII40, twelve fall inside
the published spread, five above it, and three below; of the sixteen at
HII20, seven fall inside, six above, and three below.  The lines now above
the range are the temperature-sensitive optical and ultraviolet ones, raised
with the temperature that both modern codes share ([O\,\textsc{ii}]
3726+3729, [O\,\textsc{iii}] 52+88\,$\mu$m and C\,\textsc{ii}] 2325 at
HII40; several optical lines at HII20), together with [O\,\textsc{i}]
6300+6363 at HII40 (0.0169 against 0.0059--0.012), which is fed by the thin
partly neutral edge, where the published entries themselves span a factor
two and where the cell size of a three-dimensional grid is a real
limitation.  The rows below the range are the same in the two cases:
\HeI{} 5876 and both [S\,\textsc{iii}] entries.

The clearest reading of the tables is against C25 rather than against the
25-year-old columns, and with the two temperatures now matched the line
...
$1.02\times$ C25, [N\,\textsc{iii}] 57.3\,$\mu$m $0.99\times$ and
[S\,\textsc{iii}] 18.7\,$\mu$m $1.01\times$; among the optical ones
[O\,\textsc{iii}] 5007+4959 is $0.98\times$, its auroral 4363 $0.98\times$
and [O\,\textsc{ii}] 3726+3729 $1.00\times$ --- the same optical and auroral
...
counterpart of the cooling match.  At HII20, where \MoCHII{} is $1.6\%$
hotter than C25, the same optical lines run correspondingly high,
[O\,\textsc{iii}] 5007+4959 $1.13\times$ and [O\,\textsc{ii}] 3726+3729
$1.10\times$.  The residual outliers at HII40 are not temperature:
[Ne\,\textsc{iii}] 15.5\,$\mu$m ($1.31\times$) and 3869+3968 ($1.24\times$),
[O\,\textsc{i}] 6300+6363 ($1.51\times$, on a line the thin neutral edge and
the grid resolution govern), and C\,\textsc{iii}] 1907+1909 ($0.66\times$),
...
\MoCHII{} 0.328, so what would be a $2.4\times$ excess against the 1990s
column is an $8\%$ difference against the current one.  Sulfur at HII20 is
the same story in the other direction: [S\,\textsc{iii}] 18.7\,$\mu$m
(0.221 against a published 0.34--0.46) and [S\,\textsc{iii}] 9532+9069
(0.321 against 0.465--0.604) fall \emph{below} every published code, and so
does C25 (0.209 and 0.293), while [S\,\textsc{ii}] 6716+6731 sits inside the
published range for both (0.808 and 0.744).  Both modern calculations
therefore move the same S\,\textsc{ii}/S\,\textsc{iii} split in the same
direction relative to the 2000-era compilation, which points at the sulfur
...
as a current \textsc{Cloudy}, and its weighted temperature matches that
\textsc{Cloudy} c25.00 to $0.4$~K at HII40 and $1.6\%$ at HII20 --- both
modern codes running above the quarter-century-old published temperatures,
as the atomic data of the two eras differ.  The cooling treatment that
...
MW dust gives $R_{\mathrm{eff}} = 2.206$~pc versus the reference 2.198~pc
($+0.34\%$), reproducing the classic dusty reduction $R/R_0 = 0.724$
...
2.206~pc, is indistinguishable from the absorption-only value: the in-band
...
projection maps (the emissivity integrated along the line of sight into a
surface brightness) sharing one brightness scale.  Left: PAHs surviving
...
solve, as two-dimensional projection maps of the surface brightness (left,
middle) with their azimuthally averaged radial profiles (right).  The short
...
and metal ionization structure --- and the line cooling taken from the
level populations at the local electron density, the
$n_{\mathrm{p}}n_{\mathrm{e}}$-weighted temperature is 8210~K at HII40 and
7024~K at HII20, matching a current \textsc{Cloudy} c25.00 run of the same
two models (8210 and 6910~K) to $0.4$~K and $1.6\%$; both models are
radiation-bounded, so both outer radii are predicted, and
$L(\mathrm{H}\beta)$ follows the recombination budget to within $8\%$.  Both
modern codes run above the 2000-era published temperatures (7720--8199 and
6402--6749~K), whose columns carry the atomic data of their time.  The
...
The \textsc{Cloudy} c25.00 column carried in both tables also shows how far
the reference has moved in a quarter of a century --- it sits above every
published entry in temperature --- and that c25.00 moves the same
...
(WME), a current \textsc{Cloudy}~c25.00 run (C25), and the \MoCHII{} column.
