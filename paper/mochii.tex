...
(2023) radiative and dielectronic recombination with a Mao \& Kaastra
(2016) ground-level split for case A/B, Voronov collisional ionization,
...
attenuation (rate-integral bias $+0.004\%$), the Str\"omgren sphere
(ionized volume $+0.56\%$ versus an exact 1D reference; AMR identical to
a uniform grid), a thermal H/He nebula (median $0.14\%$ in $\Te$), and
the Lexington 2000 \HII{} region benchmarks, on which \MoCHII{} lies
within the published multi-code spread: the
$n_{\mathrm{p}}n_{\mathrm{e}}$-weighted temperature is 7894~K at HII40,
inside the published 7720--8199~K, and 6376~K at HII20, $0.4\%$ below the
published floor; the outer radius and the He$^{+}$/H$^{+}$ ratio fall
inside the published ranges at both, as do 22 of the 36 tabulated line
entries, and $L(\mathrm{H}\beta)$ is within $3\%$ of them.  The
line-cooling data are
cross-validated channel by channel against a \textsc{Cloudy} c25.00
cooling decomposition on a fixed
$(\Te, \nelec, \mathrm{ionization})$ state, where the summed line cooling
agrees to $1\%$.  Both benchmarks sit at the cool edge of the published
range, which is the signature of the one documented limitation of the
thermal loop: its cooling function is the zero-density limit, so the
far-infrared fine-structure lines of low critical density
([O\,\textsc{iii}] 52 and 88\,$\mu$m, [C\,\textsc{ii}] 158\,$\mu$m) are
not collisionally suppressed and overcool the line budget by $12\%$ at
$\nelec = 110\,\mathrm{cm^{-3}}$; a density-dependent
$\Lambda(T, \nelec)$ is the planned extension.
Because the published columns are a quarter of a century old, we also run
\textsc{Cloudy} c25.00 on the same two models and carry it as an extra
column: it is hotter than every published entry, \MoCHII{} sits 4 and 8\%
below it, and several of the largest line-ratio differences (notably the
S\,\textsc{ii}/S\,\textsc{iii} split) prove to be drift in the published
reference rather than in either modern code.  We further demonstrate
...
On top of that engine \MoCHII{} adds: (i) an ionizing frequency band with
threshold-aligned bins and a zero-variance direct-field estimator;
...
microphysics (larger ion inventories, density-dependent cooling,
...
CHIANTI v11 collision strengths, and Mao \& Kaastra (2016)
level-resolved radiative recombination --- the same modern data that
\textsc{Cloudy} uses --- rather than the older compilations still carried
by \textsc{Mocassin}.

The metal-treatment axis is the clearest algorithmic difference.
\textsc{Mocassin}, \textsc{M}$^{3}$, \textsc{CMacIonize}, and the 1D codes
solve a \emph{full network}: the ionization of every element is one coupled
nonlinear system in which the metal opacity consumes photons from the
radiation field, the metal electrons enter $\nelec$, and the metal
photoheating enters the thermal balance, all iterated to self-consistency
(radiation field $\leftrightarrow$ all ions $\leftrightarrow$ $\nelec$
$\leftrightarrow$ $T$).  \MoCHII{} instead adopts a \emph{trace-species}
treatment: because the metals are present at abundances
$\sim$$10^{-4}$--$10^{-3}$, they are separated as a perturbation on the
H/He (and dust) skeleton --- the radiation field and $\nelec$ are set by
H, He, and dust, and each element's ionization cascade is an independent,
effectively linear chain evaluated on the converged
$(J_\nu, \nelec, T)$.  The one feedback that is \emph{not} dropped is metal
line cooling, which enters the thermal balance because the \HII-region
temperature is itself largely the result of metal cooling
(Section~\ref{sec:cooldata}).  Where the trace separation genuinely breaks
down, it is restored selectively by switches (the default is trace, with
recoupling only where the physics demands it): \code{ion\_metal\_abs} adds
the metal photoionization opacity to the band (on by default),
\code{metal\_ne} feeds the metal electrons into the $\nelec$ closure ---
essential in neutral photodissociation-region gas, where carbon dominates
the electron budget, and which the calculation of Section~\ref{sec:pdr}
uses --- and \code{metal\_heat} adds metal photoheating to the thermal
balance.  The design payoff is that, with each element an independent
linear chain, adding an ion is a data-file addition rather than a change
to a coupled solver (the registry of Section~\ref{sec:atomic}), and the
cell solve stays at the level of closed-form evaluations rather than a
coupled-network iteration.
...
more complete.\label{tab:compare}}
\tablewidth{0pt}
...
Recombination data & Badnell 2023 + Mao \& Kaastra 2016 & Verner \& Ferland 1996 & MAPPINGS V & Verner / Badnell & Badnell / internal \\[2pt]
Metal treatment & trace-species registry (data-driven) & full network & full MAPPINGS network & full network & full network \\[2pt]
...
[13.598, 100]$~eV in $n_\nu$ bins (typically 32).  The source spectrum ---
a Planck function $B_\nu(T_\star)$ or a tabulated spectrum in shape or
physical (per-eV, per-Hz, per-\AA, per-$\mu$m) units --- is integrated on
a sub-grid within each bin and normalized so that $\sum_b L_b =
L_{\mathrm{band}}$.  Packets sample the bin from the luminosity CDF and
carry $L_{\mathrm{pkt}} = L_{\mathrm{band}}/N$.

\paragraph{Threshold-aligned bins.}  By default (\code{ion\_align\_edges})
...
largest lever on the He and metal ionization structure in the HII40
benchmark (Section~\ref{sec:hii}).  When thresholds outnumber bins the
...
external field is a band-integrated mean intensity $J$ entering through
...
\alpha_B$ and photon energies $E = E_{\mathrm{th}} + kT_e\,x$, $x \sim
\mathrm{Exp}(1)$.  The emission table is rebuilt from the current state
...
threshold.  An optional fourth channel adds the \HeI{} excited-level
recombination radiation with the low-density branching of
\citet{osterbrock2006}.  At HII40 the diffuse band luminosity converges to
$\sim$$0.5\,L_\star$.

\subsection{Quasi-random photon launching}
\label{sec:qmc}

Optionally the packet launch draws its variables --- source component,
frequency bin, direction, and any entry-surface coordinates --- from an
...
function of the launch variables (Eq.~\ref{eq:edgewalk}), so a
low-discrepancy launch set reduces its noise directly: on the fixed-state
analytic-attenuation test the cell-wise $\Gamma_{\mathrm{HI}}$ error scales
as $\sim$$1/N$ (Monte Carlo $1/\sqrt{N}$), with effective photon gains of
$29\times$ at $N = 2^{17}$ and $222\times$ at $2^{20}$, at unchanged wall
time and no significant bias.
...
\begin{gather}
  ...
  \label{eq:badnellrr}
\end{gather}%
the ground-level ($n=1$) direct-capture rate $\alpha_1$ from the
level-resolved parameterization of \citet{mao2016}, and forms case B by
removing the ground term, $\alpha_B = \alpha_A - \alpha_1$.  For the
H-like ions this $\alpha_1$ is derived from the exact hydrogenic cross
sections (Eq.~\ref{eq:sighyd}) under detailed balance, so it is exact; the
$\HeII\to\HeI$ channel adds the Badnell three-term dielectronic sum.  This
...
$10^4$~K) is $\sim$7\% higher and coincides with \textsc{Cloudy}, while
the older \citet{huignedin1997} value is low and the
...
\citet{dere2007}/Voronov ratio per (element, stage); the difference matters
...
closed-form fit; the diagnostic output needs density-dependent
emissivities, and therefore solves the level populations.
...
The zero-density limit is a deliberate approximation with a stated domain
of validity: $\Lambda_{\mathrm{ion}}(T)$ carries no collisional
de-excitation, so it is exact for lines whose critical density is far
above the local $\nelec$ and increasingly too large for those below it.
The affected lines are the far-infrared fine-structure transitions,
[O\,\textsc{iii}] 52 and 88\,$\mu$m and [C\,\textsc{ii}] 158\,$\mu$m,
whose electron critical densities are $10^{2}$--$10^{3.5}\,
\mathrm{cm^{-3}}$, so at
the $\nelec \sim 10^2\,\mathrm{cm^{-3}}$ of an \HII{} region the fit
overestimates the line-cooling budget by about $12\%$
(Section~\ref{sec:coolcheck}), while it is accurate in the diffuse gas the
limit is written for.  A density-dependent $\Lambda(T, \nelec)$ table,
which the same pipeline can produce and which the planetary-nebula
densities of Section~\ref{sec:summary} require in any case, is the planned
replacement.
...
option (the two agree to a median $5\times10^{-5}$ in $\Te$).  A He\,\textsc{i}
$2^3$S metastable diagnostic, including the 10830~\AA\ optical depth, is
also available.
...
$\Gamma_{\mathrm{HI}}$ has a mean bias of $+0.004\%$ and a median deviation
...
4\pi)^{1/3} = 3.0365$~pc; a uniform level-6 octree gives 3.0534~pc
($+0.56\%$), and a front-refined octree with $3.5\times$ fewer cells
reproduces it to four decimals, while a center-refined control (coarsest
cells at the front) deviates by $+2.35\%$ --- the ionization front is the
one place needing resolution (Figure~\ref{fig:g1}).  With the thermal
balance on, $\Te(r)$ matches the 1D thermal reference to a median $0.14\%$
and a mean bias $+0.006\%$ (Figure~\ref{fig:g2a}); without metals the
...
(HII40) and $1.004\times10^{49}\,\mathrm{s^{-1}}$ (HII20); the band carries
32 threshold-aligned bins (Section~\ref{sec:band}); and the diffuse field is
transported explicitly with case-A rates and the \HeI{} channels
...
octree ($64^3$ leaves) run with $4\times10^{7}$ stellar packets to the
volume-integrated convergence criterion.

Those published columns are, however, a quarter of a century old, and one of
them (GF) is an early version of a code that is still developed.  To separate
what has changed in \MoCHII{} from what has changed in the reference itself,
we also ran \textsc{Cloudy} c25.00 \citep{cloudy2025} on both benchmarks and
carry it as the C25 column of the same two tables.  Its physics input is the
...
to how the ionizing band is discretized at the \HeI{} edge.  On a plain
logarithmic grid that edge is straddled and the He-ionizing photon budget is
overdelivered (Section~\ref{sec:band}); pinning the bin edges on the
thresholds returns that budget to the exact blackbody value and, with every
other setting held fixed, moves the mean \HeI{} fraction from 0.058 to
0.163 and C\,\textsc{iii} from 0.786 to 0.694, with O\,\textsc{iii} and
N\,\textsc{iii} dropping correspondingly.  It is the single largest lever on
the ionization structure of this model, and the benchmark runs use the
aligned band throughout.  On it, the helium ratio
$\langle\mathrm{He^{+}}\rangle/\langle\mathrm{H^{+}}\rangle$ is 0.715 at
HII40, at the low end of the published 0.715--0.829 (median 0.767) and
below a current \textsc{Cloudy} (C25 0.741, itself under the published
median), and 0.047 at HII20, on the published median of 0.048 inside a
spread of 0.034--0.077.

\emph{Temperature.}  The weighted temperature is 7894~K at HII40, inside
the published 7720--8199~K and $1.6\%$ below the eight-code median of
8026~K, and 6376~K at HII20, $0.4\%$ below the published floor of 6402~K
and $4.3\%$ below the median of 6662~K.  The inner-edge temperature behaves
the same way: 7270~K at HII40 against a published 7370--8318~K and 6752~K
at HII20 inside a published 6562--7224~K.  The converged MOCASSIN 2.02 run
of the HII40 model gives 8176~K, also inside the published spread and $6\%$
above MOCASSIN's own published entry.  \MoCHII{}
therefore reproduces the published temperatures, and does so from the low
side of the distribution in both cases.

That the two benchmarks sit at the same edge is informative, because the
published columns are not themselves a stationary target.  \textsc{Cloudy}
c25.00 on the same two models gives 8210~K and 6910~K, in both cases
\emph{above} the entire published spread; measured against a contemporary
code rather than a 25-year-old table, \MoCHII{} is $3.8\%$ and $7.7\%$
cooler.  The two modern calculations thus straddle the published set from
opposite sides, and the offset between them agrees in sign and size with
the one documented approximation in the \MoCHII{} thermal loop --- the
zero-density cooling function of Section~\ref{sec:cooldata}, which
overestimates the far-infrared line cooling at
$\nelec \sim 10^{2}\,\mathrm{cm^{-3}}$ and so biases $\Te$ low.  The next
subsection measures that overestimate directly.

% Published columns: Tables 4 and 5 of Ercolano et al. (2003), transcribed
% and checked value by value against the paper.
% MoCHII column: the benchmark runs hii40_bench.in / hii20_bench.in
% (aligned bins, explicit diffuse field, Q(H) matched to the specification,
% 4e7 packets, conv_crit='vol'); line entries are the L/L(Hbeta) column of
% tests/g2_hii/hii{40,20}_bench_lines.txt (doublets summed), temperatures
% computed from tests/g2_hii/hii{40,20}_bench_rates.h5.
...
Replacing the $n$-level emissivities by the zero-density fit that the
thermal loop uses (Eq.~\ref{eq:cool}) raises the same summed line cooling
...
accounts for the temperature bias quantitatively: scaling $\Te$ on the
fixed state until the \MoCHII{} terms radiate the same total power ---
\textsc{Cloudy} balances its own heating at the unscaled temperature, and
the photoheating of the two codes agrees --- gives 7784~K against
\textsc{Cloudy}'s 8210~K, a $5.2\%$ displacement, against the $3.8\%$ by
which the converged \MoCHII{} run is cooler than C25.  A first-order
estimate at fixed ionization is thus more than sufficient to explain the
offset, and the remedy is the $\Lambda(T,\nelec)$ table of
Section~\ref{sec:cooldata} rather than a change of atomic data.
...
approximation, with every other setting fixed, gives 7570~K at HII40 and
6216~K at HII20, so the explicit field used here raises $\Te$ by $4.3\%$
and $2.6\%$ and moves the two benchmarks the same way.  Substituting
...
with the published set ([Ne\,\textsc{iii}] 15.5\,$\mu$m 0.278 in a
published 0.263--0.429; [Ne\,\textsc{ii}] 12.8\,$\mu$m 0.202 in
0.177--0.217; [O\,\textsc{iii}] 52+88\,$\mu$m 2.41 in 2.10--2.42), and so
do the temperature-sensitive optical lines of the same ion:
[O\,\textsc{iii}] 5007+4959 is 2.06 against a published 1.64--3.27, and the
auroral [O\,\textsc{iii}] 4363, the most temperature-sensitive entry in the
tables, is 0.0033 against a published 0.0022--0.0070.  The lines that fall
...
captured inside the prescribed sphere, at $1.43\times10^{19}$~cm against an
outer radius of $1.46\times10^{19}$~cm, and the outermost shell is $86\%$
...
gives $0.953\,\QH$ and $0.971\,\QH$.  What hydrogen does not absorb is
taken up by helium, which at HII40 absorbs $0.114\,\QH$ and returns most of
it to hydrogen through the \HeI{} diffuse channels.  The predicted
$L(\mathrm{H}\beta) = 1.95\times10^{37}$ and
$4.70\times10^{36}\,\mathrm{erg\,s^{-1}}$ are $5\%$ and $4\%$ below the
published medians ($3\%$ below the lowest published values), matching the
$4.7\%$ and $2.9\%$ of $\QH$ that the recombination budget assigns to
helium rather than to hydrogen.  The absolute H$\beta$ luminosity is thus
reproduced by photon counting, and adds no discrepancy of its own.
...
% the MoCHII column is computed from tests/g2_hii/hii{40,20}_bench_lines.txt.
\input{tables/wood_hii40}
\input{tables/wood_hii20}

\emph{The complete published line list.}  Tables~\ref{tab:wood40} and
\ref{tab:wood20} repeat the comparison over the full benchmark line list in
...
\MoCHII{} column is appended, and the C25 column runs beside its 1990s
ancestor GF.  Of the twenty lines tabulated at HII40, thirteen fall inside
the published spread, one above it, and six below; of the sixteen at HII20,
nine fall inside and seven below.  The one line above the published range,
[O\,\textsc{i}] 6300+6363 at HII40 (0.0158 against 0.0059--0.012), is fed
by the thin partly neutral edge, where the published entries themselves
span a factor two and where the cell size of a three-dimensional grid is a
real limitation.  The rows below the range are largely the same in the two
cases: \HeI{} 5876, [N\,\textsc{ii}] 6584+6548 and 5755, and both
[S\,\textsc{iii}] entries.

The clearest reading of the tables is the contrast between the
temperature-sensitive and the temperature-insensitive lines of the same
ion, measured against C25 rather than against the 25-year-old columns.  The
far-infrared fine-structure lines, whose excitation energies are small
enough that their emissivities barely depend on $\Te$, agree with C25 to a
few percent at both benchmarks: [O\,\textsc{iii}] 52+88\,$\mu$m is
$1.00\times$ and $1.12\times$ C25, [Ne\,\textsc{ii}] 12.8\,$\mu$m
$1.01\times$ and $0.98\times$, [N\,\textsc{iii}] 57.3\,$\mu$m $0.98\times$
and $0.95\times$, and [S\,\textsc{iii}] 18.7\,$\mu$m $0.98\times$ at both.
The temperature-sensitive optical and ultraviolet lines run low by about
what the temperature offset implies: at HII40, where \MoCHII{} is $3.8\%$
cooler than C25, [O\,\textsc{iii}] 5007+4959 is $0.84\times$ C25 and the
auroral 4363 $0.72\times$; at HII20, where the offset is twice as large,
[N\,\textsc{ii}] 5755 is $0.56\times$, [O\,\textsc{ii}] 7320+7330
$0.57\times$ and [O\,\textsc{ii}] 3726+3729 $0.63\times$.  At a given
ionization structure, infrared lines flat and optical and auroral lines low
is a temperature signature rather than an atomic-data one --- which is what
the fixed-state comparison of Section~\ref{sec:coolcheck} establishes
independently, since there the same emissivities agree with
\textsc{Cloudy}'s to $1\%$ when both are evaluated on one state.

The C25 column also shows how much of the disagreement with the published
columns is a property of those columns.  [S\,\textsc{ii}] 6716+6731 at
HII40 is the clearest case: the GF entry is 0.137, C25 gives 0.304 and
\MoCHII{} 0.299, so what would be a $2.2\times$ excess against the 1990s
column is a $2\%$ difference against the current one, and both modern
values sit inside the published spread only because one 2000-era entry
(TK, 0.315) is high.  Sulfur at HII20 is the same story in the other
direction: [S\,\textsc{iii}] 18.7\,$\mu$m (0.204 against a published
0.34--0.46) and [S\,\textsc{iii}] 9532+9069 (0.238 against 0.465--0.604)
fall \emph{below} every published code, and so does C25 (0.209 and 0.293),
while [S\,\textsc{ii}] 6716+6731 sits inside the published range for both
(0.592 and 0.744).  Both modern calculations therefore move the same
S\,\textsc{ii}/S\,\textsc{iii} split in the same direction relative to the
2000-era compilation, which points at the sulfur data rather than at either
code.  A band experiment bounds one further
...
0.15~pc of one another near 3.9~pc, C$^{+3}$ stays below $0.05$ and is
...
1.4~pc while hydrogen stays ionized out to 2.8~pc, so the doubly ionized
...
$R_{\mathrm{out}}$ are $1.428$ and $1.434\times10^{19}$~cm at HII40 against
a published 1.45--$1.47\times10^{19}$, and $8.58$ and
$8.69\times10^{18}$~cm at HII20 against a published
8.7--$9.01\times10^{18}$: the two definitions of the outer boundary are not
identical, but they locate the same front, and both sit $1$--$2\%$ below
the published ranges, which is what the recombination budget of
$0.95$--$0.97\,\QH$ and the slightly larger $\alpha_B$ of a cooler nebula
together imply.  The temperature panels
...
over the full box with the source at the center, and $4\times10^7$ packets.
...
% source: tests/g2_hii/hii40_bench_rates.h5 and the Cloudy c25.00 save files
...
\MoCHII{} column of Table~\ref{tab:wood40}, on the threshold-aligned
32-bin ionizing grid.  Black lines are \textsc{Cloudy}~c25.00 on the same
...
% source: tests/g2_hii/hii20_bench_rates.h5 and the Cloudy c25.00 save files
...
which is why the aligned ionizing grid matters much less here than at
HII40.  Black lines are \textsc{Cloudy}~c25.00 on the same model, from the
...
\emph{How far the reference itself has moved.}  Because the C25 column is a
re-run of one of the participating codes on the same two models, the
difference between it and the GF column measures the drift of the reference
over a quarter of a century, at fixed input.  That drift is not small.  The
absolute $L(\mathrm{H}\beta)$ is stable --- 2.06 against 2.06 at HII40, 4.99
against 4.85 at HII20, in the units of the tables --- as it must be, since it
is fixed mostly by the photon budget.  The temperature has risen by 270~K at
HII40 and 230~K at HII20.  The line ratios have moved much further: at HII40
[S\,\textsc{ii}] 6716+6731 is $2.2\times$ its GF value, [O\,\textsc{iii}]
4363 $2.0\times$, and [Ne\,\textsc{iii}] 3869+3968 $0.69\times$; at HII20 the
two [S\,\textsc{iii}] entries fall to $0.47$ and $0.58$ of their GF values
while [S\,\textsc{ii}] 6716+6731 rises by $1.5\times$.  A benchmark column
published in 2000 is therefore not a fixed target for a code written in 2026,
and a modern code that reproduced the GF entries exactly would be
reproducing superseded atomic data.  This is the reason the comparison above
is made against the published \emph{spread} and against C25 rather than
against any single published column.
...
temperature is correspondingly at or below the low end of the published
...
as a current \textsc{Cloudy}, and its weighted temperature is inside the
spread at HII40 and $0.4\%$ under it at HII20.  What remains is a coherent
and quantified bias rather than an open discrepancy: \MoCHII{} runs
$4$--$8\%$ cooler than a contemporary \textsc{Cloudy} on both models, in
the direction and by about the amount that the zero-density cooling
function of Section~\ref{sec:cooldata} implies, and the density-dependent
$\Lambda(T,\nelec)$ that removes it is the next item for the thermal
balance.
...
$4.5\times$ fewer cells, and both improve on level 6 ($+0.56\%$), as
...
$\Te = 7502$~K, and the refinement (32\,768 $\to$ 617\,324 leaves) tracks
...
$C_{\mathrm{abs}}/\mathrm{H} = 1.83\times10^{-21}$ at 13.6~eV falls to
...
MW dust gives $R_{\mathrm{eff}} = 2.246$~pc versus the reference 2.231~pc
($+0.69\%$), reproducing the classic dusty reduction $R/R_0 = 0.735$
(Figure~\ref{fig:duststrom}); dust absorbs 59\% of the band luminosity
there.
...
2.237~pc, lands inside the analytic bracket (2.086~pc if the dust
extinction is all absorption, 2.246~pc for absorption only) and close to
the absorption-only bound, exactly as strong forward scattering predicts.
...
pushed inward to $R_{\mathrm{eff}} = 2.25$~pc, the classic dust-limited
size ($R/R_0 = 0.736$).  Under \code{laursen09\_live}, by contrast, the
...
ionization front inward to $R/R_0 = 0.736$, while the live
...
89~K at 0.3~pc to 28~K at the sphere edge (the canonical \HII-region
...
ionization-dependent PAH share.  Both panels are two-dimensional projection
maps --- the emissivity integrated along the line of sight into a surface
brightness --- and share one brightness scale.  Left: PAHs surviving
everywhere, giving a centrally peaked image.  Right: PAHs destroyed in the
ionized interior ($x_{\mathrm{HII}}$-weighted survival).  The central peak
disappears and the projected brightness instead rises outward to a broad
limb-brightened annulus just inside the ionization front, a factor 1.9 above
...
solve, as two-dimensional projection maps of the surface brightness.  The
short wavelength is centrally peaked (warm inner dust) while the
far-infrared is spatially broader, the projected signature of the dust
...
temperature settles to a genuine PDR equilibrium (a $\sim$336~K to
$\sim$211~K gradient, not pinned at the temperature floor).  This is the
...
1D codes.  It reproduces analytic attenuation to $+0.004\%$, the Str\"omgren
sphere to $+0.56\%$, and a thermal H/He nebula to $0.14\%$.  On the
Lexington 2000 \HII{} region benchmarks, with the ionizing bins aligned on
the ionization thresholds --- the discretization that controls the helium
and metal ionization structure --- it lies inside the published multi-code
spread: the $n_{\mathrm{p}}n_{\mathrm{e}}$-weighted temperature is 7894~K
at HII40 within a published 7720--8199~K and 6376~K at HII20 just under a
published floor of 6402~K; both models are radiation-bounded, so both outer
radii are predicted, and $L(\mathrm{H}\beta)$ follows the recombination
budget to a few percent.  The line-cooling data are checked independently
of the temperature by evaluating the \MoCHII{} cooling terms on a fixed
state taken from a \textsc{Cloudy} c25.00 cooling decomposition: channel by
channel the two compilations agree, and the summed line cooling agrees to
$1\%$.  The one documented approximation left in the thermal loop is the
zero-density cooling function, which through the far-infrared
fine-structure lines overestimates the total line cooling by $12\%$ at
$\nelec = 110\,\mathrm{cm^{-3}}$ and
places both benchmarks at the cool edge of the published range, $4$--$8\%$
below a \textsc{Cloudy} c25.00 run of the same two models carried as an
extra column of both tables; a density-dependent $\Lambda(T,\nelec)$ is the
planned replacement.  That comparison column also shows how far the
reference has moved in a quarter of a century: c25.00 sits above every
published entry in temperature, and it moves the same
S\,\textsc{ii}/S\,\textsc{iii} rows in the same direction \MoCHII{} does.
The solution-driven
...
synthetic 10830~\AA\ absorption modeling.  A hard-spectrum extension for
planetary nebulae and active galactic nuclei --- higher photon energies,
extended ion stages, inner-shell/Auger processes, and density-dependent
cooling tables --- is planned.  The trace-metal registry makes further
...
% TODO: acknowledgments -- funding (KASI/NRF grant numbers), the MOCASSIN,
% CHIANTI, Cloudy, PyNeb, and Illustris-TNG teams for public data and
% comparison references, and computing resources.
\end{acknowledgments}
