...
\textsc{Cloudy} c25.00 calculation of the same models to $0.2\%$ (HII40)
...
against the tabulated $1.58 \times 10^{-13}$; the code reports that integral
against the $\alpha_A - \alpha_B$ used for the emission rate without
rescaling either, and they agree to $0.5$--$1.3$\% for \HI{} and $0.4$\%
for \HeII{} while differing by up to $3.4$\% for \HeI{}.  Because packets
...
recombination rate by $3\%$, the size of the entire spread among published
determinations, moves the front by $0.008$ and $0.024$~pc, so $0.18$~pc would
take $70\%$ and $23\%$.  Outside the diffuse field, the source photon budget
reproduces the \textsc{Cloudy} $Q({>}13.6~\mathrm{eV})$ and
$Q({>}24.6~\mathrm{eV})$ to $0.003\%$, and the benchmark runs dust free.
...
24.6~eV.  Closing $4.4\%$ in radius is $13.7\%$ in volume, asking for $14\%$
in $Q(\mathrm{He^{0}})$ or $20\%$ in $\alpha_B$(\HeI), and neither is
available.  The residual is thus unexplained while the simple explanations
for it are quantitatively excluded; the candidate left is the helium
ionization network itself --- what \textsc{Cloudy}'s resolved \HeI{} model
atom contains that an effective branch treatment does not --- which calls for
a network-to-network code comparison.  At HII40 the diffuse band luminosity
...
\emph{Temperature.}  The weighted temperature is 8162~K at HII40 and
6918~K at HII20.  Both match the \textsc{Cloudy} c25.00 solution of the
same two models --- 8210~K and 6910~K --- to $0.6\%$ and $0.1\%$, and both
...
published spread at both, 7533~K at HII40 against a published 7370--8318~K
and 6936~K at HII20 against a published 6562--7224~K.  The converged
...
$\nelec$, Section~\ref{sec:cooldata} --- gives the 8162 and 6918~K that
...
default reaches 8162~K, matching \textsc{Cloudy} to $0.6\%$
...
\MoCHII{} 0.322; the two latter values differ by $6\%$.  Sulfur at HII20 is
...
published range for both (0.809 and 0.744).  Both C25 and \MoCHII{} have the
...
solution to $0.2\%$ at HII40 and $0.1\%$ at HII20.  The cooling treatment that
...
density, the
$n_{\mathrm{p}}n_{\mathrm{e}}$-weighted temperature is 8162~K at HII40 and
6918~K at HII20, matching the \textsc{Cloudy} c25.00 solution of the same
two models (8210 and 6910~K) to $0.6\%$ and $0.1\%$; both models are
