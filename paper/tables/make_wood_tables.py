...
* The MoCHII line column is computed here from the converged line tables of
  the threshold-aligned benchmark runs,

      ../../tests/g2_hii/hii40_bench_lines.txt
      ../../tests/g2_hii/hii20_bench_lines.txt

  (4e7 packets, level-6 octree, conv_crit='vol'), by summing exactly the
...
* The two MoCHII temperatures are computed here from the rates files of the
  same runs,
      ...
      ../../tests/g2_hii/hii40_bench_rates.h5
      ../../tests/g2_hii/hii20_bench_rates.h5
...
def mochii_tnpne(fn):
    ...
    the closure also used by tests/g2_hii/check_g2_hii.py.
    """
    import h5py
    import numpy as np
...
The \MoCHII{} column uses threshold-aligned 32 ionizing bins, the explicit
Monte Carlo diffuse field including the \HeI{} channels (case~A, matching
the transfer treatment of the published codes), and Badnell 2023
recombination with the Mao \& Kaastra 2016 ground-level split, at
$4\times10^{7}$ stellar packets with the volume-integrated convergence
criterion; the ionizing luminosity is set from the benchmark $L({\rm BB})$
so that $Q({\rm H})$ matches the specification; blends are the sum of the
...
def emit(tag, table, lines_file):
    lhb, rows = read_lines(os.path.join(TESTS, lines_file))
    cl = read_cloudy(tag)
    rates = os.path.join(TESTS, "%s_bench_rates.h5" % tag)
...
emit("hii40", HII40, "hii40_bench_lines.txt")
emit("hii20", HII20, "hii20_bench_lines.txt")
