...
DDIR = "../../tests/g1_stromgren/"
runs = {}
for tag, fname, hcell in (("uniform L6", DDIR + "g1_stromgren_rates.h5", None),
                          ("refined 4-6", DDIR + "g1_stromgren_ref_rates.h5", None),
                          ("front 4-6", DDIR + "g1_stromgren_front_rates.h5", None)):
    try:
        run = load_run(fname)
...
# Distinct color AND marker per grid (colorblind-safe): filled blue circle,
# open red square, open green triangle.  Draw the densest (uniform L6) first.
# uniform L6 and front 4-6 trace the SAME sharp front (both R_eff = 3.0534
# pc), so they overlap tightly; refined 4-6 is the deviating grid (partially
# ionized outliers).  Blue = dense filled base, green = sparse open rings on
# top (so the blue base shows through), red = open squares for the outliers.
styles = {
    "uniform L6":  dict(marker="o", ms=2.6, mfc="tab:blue", mec="none",
                        ls="none", alpha=0.55, zorder=1),
    "front 4-6":   dict(marker="^", ms=1.7, mfc="none", mec="tab:green",
                        mew=0.4, ls="none", alpha=0.30, zorder=2),
    "refined 4-6": dict(marker="s", ms=3.0, mfc="none", mec="tab:red",
                        mew=0.7, ls="none", alpha=0.85, zorder=3),
}
# Random 1-in-THIN sample of the Monte Carlo cells for plotting; every quoted
# number is computed from the full cell set.  The same sample is reused in
# both panels so the two views show the identical cells.
rng = np.random.default_rng(20260725)
picks = {}
for tag in runs:
    gas = np.nonzero(runs[tag][0]["ne"] > 0)[0]
    picks[tag] = rng.choice(gas, size=max(1, gas.size // THIN), replace=False)

for tag in ("uniform L6", "refined 4-6", "front 4-6"):
    if tag not in runs:
        continue
    run, half = runs[tag]
    sel = picks[tag]
    ax1.plot(run["r"][sel], run["xHI"][sel], label=tag, rasterized=True,
             **styles[tag])
...
for tag in ("uniform L6", "refined 4-6", "front 4-6"):
    ...
    run, half = runs[tag]
    sel = picks[tag]
    ax2.plot(run["r"][sel], run["xHeI"][sel], rasterized=True, **styles[tag])
    ax2.plot(run["r"][sel], run["xHeII"][sel], rasterized=True, **styles[tag])
...
fig.tight_layout()
fig.savefig(_os.path.join(FIGDIR, "g1_stromgren_check.pdf"), dpi=200, bbox_inches="tight")
print("wrote g1_stromgren_check.pdf")
