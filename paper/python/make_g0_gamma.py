...
# ---------------------------------------------------------------- main ---
f = h5py.File("../../tests/g0_gamma/g0_gamma_rates.h5", "r")
...
# Random 1-in-THIN sample of the cells for plotting (sorted in radius so the
# analytic curve stays smooth); the printed comparison uses every cell.
rng = np.random.default_rng(20260725)
_pick = rng.choice(r_pc.size, size=max(1, r_pc.size // THIN), replace=False)
...
for name, sig, c in (("HI", sig_HI, "tab:blue"), ("HeI", sig_HeI, "tab:red")):
    ...
    ax1.plot(r_pc[order], ana_c[order], "k-", lw=0.8, rasterized=True,
             label="analytic" if name == "HI" else None)
    ax2.plot(r_pc[order], g_mc[name][order]/ana_c[order], ".", ms=1.5,
             color=c, alpha=0.3, label=name, rasterized=True)
ax1.set_yscale("log")
ax1.set_ylabel(r"$\Gamma$ [s$^{-1}$]")
ax1.set_title(r"Photoionization rate $\Gamma(r)$")
ax1.legend(frameon=False, markerscale=6, loc="upper right")
...
ax2.legend(frameon=False, markerscale=4)
fig.tight_layout()
fig.savefig(_os.path.join(FIGDIR, "g0_gamma_check.pdf"), dpi=200, bbox_inches="tight")
print("\nwrote g0_gamma_check.pdf")
