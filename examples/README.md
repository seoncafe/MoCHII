...
| `hei_metastable` | He I 2^3S metastable diagnostic (`par%hei_metastable`): the 2^3S population, the 10833 A line-center absorption opacity, and the 2^3S photoionization rate. Three runs contrast the low-density radiative limit, the high-density collisional plateau, and soft-UV (`add_fuv`) depletion of the metastable reservoir near the star. The switch adds only the diagnostic output; the physics is unchanged. |
